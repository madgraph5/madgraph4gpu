...
import logging
logger = logging.getLogger('madgraph.PLUGIN.ALPAKA_CUDACPP_SA_OUTPUT.model_handling')
...
class PLUGIN_ALOHAWriter(aloha_writers.ALOHAWriterForGPU):
    ...
    def get_header_txt(self, name=None, couplings=None,mode=''):
        ...
        out.write('  %(comment)s\n#ifdef ALPAKA\n  template< typename T_Acc >\n  ALPAKA_FN_ACC %(prefix_alp)s\n  void %(name)s( __attribute__((unused)) T_Acc const &acc,\n#else\n  %(prefix)s\n  void %(name)s( \n#endif\n' %
                  {'comment': comment, # AV - add comment
                   'prefix': self.prefix + ( ' INLINE' if 'is_h' in mode else '' ), # AV - add INLINE
                   'prefix_alp': ( ' INLINE' if 'is_h' in mode else '' ), # AV - add INLINE
                   'name': name})
        out.write('%(indent)sconst %(args)s,\n%(indent)s%(output)s )%(suffix)s' %
                  {
...
class PLUGIN_GPUFOHelasCallWriter(helas_call_writers.GPUFOHelasCallWriter):
    ...
    def super_get_matrix_element_calls(self, matrix_element, color_amplitudes):
        ...
        for diagram in matrix_element.get('diagrams'):
            ...
            for wf in diagram.get('wavefunctions'):
              func = self.get_wavefunction_call(wf)
              if not func.startswith('#ifdef ALPAKA'):
                func_alp = func.replace('( ','( acc, ', 1)
                res.append('#ifdef ALPAKA')
                res.append(func_alp)
                res.append('#else')
                res.append(func)
                res.append('#endif')
            ...
            for amplitude in diagram.get('amplitudes'):
                ...
                func = self.get_amplitude_call(amplitude)
                if not func.startswith('#ifdef ALPAKA'):
                  func_alp = func.replace('( ','( acc, ', 1)
                  res.append('#ifdef ALPAKA')
                  res.append(func_alp)
    ...
    def get_external(self, wf, argument):
        ...
        split_line_alp = split_line.copy() # DS
        split_paren = split_line_alp[0].split('(')
        split_paren[1] = 'acc, ' + split_paren[1]
        split_line_alp[0] = '('.join(split_paren)
        line_alp = ', '.join(split_line_alp) # DS (for ALPAKA)
        ###split_line.insert(-1, ' ievt')
        split_line_cpp = split_line.copy()
        split_line_cpp.insert(-1, 'ipagV') # AV (for C++)
        if self.first_get_external and ( ( 'mzxxx' in line ) or ( 'pzxxx' in line ) or ( 'xzxxx' in line ) ) :
            ...
            split_line2_alp = split_line2.copy()
            split_paren = split_line2_alp[0].split('(')
            split_paren[1] = 'acc, ' + split_paren[1]
            split_line2_alp[0] = '('.join(split_paren)
            line2_alp = ', '.join(split_line2_alp) # DS (for ALPAKA)
            text = '#ifdef ALPAKA\n#ifndef MGONGPU_TEST_DIVERGENCE\n      %s\n#else\n      if ( ( blockDim.x * blockIdx.x + threadIdx.x ) %% 2 == 0 )\n        %s\n      else\n        %s\n#endif\n#elif defined(__CUDACC__)\n#ifndef MGONGPU_TEST_DIVERGENCE\n      %s\n#else\n      if ( ( blockDim.x * blockIdx.x + threadIdx.x ) %% 2 == 0 )\n        %s\n      else\n        %s\n#endif\n#else\n      %s\n#endif\n' # AV
            return text % (line_alp, line_alp, line2_alp, line, line, line2, ', '.join(split_line_cpp))
        ###text = '\n#ifdef __CUDACC__\n    %s    \n#else\n    %s\n#endif \n'
        text = '#ifdef ALPAKA\n      %s\n#elif defined(__CUDACC__)\n      %s\n#else\n      %s\n#endif\n' # AV
        return text % (line_alp, line, ', '.join(split_line_cpp))
