...
#pragma omp parallel for default(none) shared(allmomenta,allMEs,cHel,cIPC,cIPD,ihel,npagV) private (amp_sv,w_sv,jamp_sv)
...
#endif
    {
      ...
      opzxxx(acc,  allmomenta, cHel[ihel][0], -1, w_sv[0], 0 ); // NB: opzxxx only uses pz
#else
      if ( ( blockDim.x * blockIdx.x + threadIdx.x ) % 2 == 0 )
        opzxxx(acc,  allmomenta, cHel[ihel][0], -1, w_sv[0], 0 ); // NB: opzxxx only uses pz
      else
        oxxxxx(acc,  allmomenta, 0, cHel[ihel][0], -1, w_sv[0], 0 );
#endif
#elif defined(__CUDACC__)
#ifndef MGONGPU_TEST_DIVERGENCE
...
#ifdef ALPAKA
      imzxxx(acc,  allmomenta, cHel[ihel][1], +1, w_sv[1], 1 ); // NB: imzxxx only uses pz
#elif defined(__CUDACC__)
...
#ifdef ALPAKA
      ixzxxx(acc,  allmomenta, cHel[ihel][2], -1, w_sv[2], 2 );
#elif defined(__CUDACC__)
...
#ifdef ALPAKA
      oxzxxx(acc,  allmomenta, cHel[ihel][3], +1, w_sv[3], 3 );
#elif defined(__CUDACC__)
...
#ifdef ALPAKA
      FFV1P0_3( acc, w_sv[1], w_sv[0], cxmake( cIPC[0], cIPC[1] ), 0., 0., w_sv[4] );
#else
...
#ifdef ALPAKA
      FFV1_0( acc, w_sv[2], w_sv[3], w_sv[4], cxmake( cIPC[0], cIPC[1] ), &amp_sv[0] );
#else
...
#ifdef ALPAKA
      FFV2_4_3( acc, w_sv[1], w_sv[0], cxmake( cIPC[2], cIPC[3] ), cxmake( cIPC[4], cIPC[5] ), cIPD[0], cIPD[1], w_sv[4] );
#else
...
#ifdef ALPAKA
      FFV2_4_0( acc, w_sv[2], w_sv[3], w_sv[4], cxmake( cIPC[2], cIPC[3] ), cxmake( cIPC[4], cIPC[5] ), &amp_sv[0] );
#else
