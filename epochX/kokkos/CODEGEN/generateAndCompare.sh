...
function codeGenAndDiff()
{
  ...
  if [ "${SCRBCK}" == "gridpack" ] && [ "${UNTARONLY}" == "1" ]; then
    ...
    if [ "${SCRBCK}" == "gridpack" ]; then # $SCRBCK=$OUTBCK=gridpack
      ...
      echo "output standalone_${SCRBCK}_kokkos ${outproc}" >> ${outproc}.mg
    elif [ "${OUTBCK}" == "madonly" ]; then # $SCRBCK=kokkos and $OUTBCK=madonly
      echo "output madevent ${outproc} ${helrecopt} --vector_size=${vecsize}" >> ${outproc}.mg
    elif [ "${OUTBCK}" == "mad" ]; then # $SCRBCK=kokkos and $OUTBCK=mad
      echo "output madevent ${outproc} ${helrecopt} --vector_size=${vecsize} --me_exporter=standalone_kokkos" >> ${outproc}.mg
    elif [ "${OUTBCK}" == "madcpp" ]; then # $SCRBCK=kokkos and $OUTBCK=madcpp
      echo "output madevent ${outproc} ${helrecopt} --vector_size=32 --me_exporter=standalone_cpp" >> ${outproc}.mg
    elif [ "${OUTBCK}" == "madgpu" ]; then # $SCRBCK=kokkos and $OUTBCK=madgpu
      echo "output madevent ${outproc} ${helrecopt} --vector_size=32 --me_exporter=standalone_gpu" >> ${outproc}.mg
    else # $SCRBCK=kokkos and $OUTBCK=kokkos, cpp or gpu
  ...
  # Output directories: examples ee_mumu.sa for kokkos, eemumu.auto for alpaka and gridpacks, eemumu.cpp or eemumu.gpu for cpp and gpu
  autosuffix=sa
  if [ "${SCRBCK}" == "gridpack" ]; then
    autosuffix=auto
  ...
  # Additional patches for mad directory (integration of Fortran and kokkos)
  ...
  if [ "${OUTBCK}" == "kokkos" ] || [ "${OUTBCK}" == "gridpack" ]; then
    pushd ${OUTDIR} >& /dev/null
...
function usage()
{
  ...
  if [ "${SCRBCK}" == "gridpack" ]; then
    ...
    # NB: all options with $SCRBCK=kokkos use the 311 branch by default and always disable helicity recycling
...
OUTDIR=$(dirname $SCRDIR) # e.g. epochX/kokkos if $SCRDIR=epochX/kokkos/CODEGEN

# Script directory backend (kokkos, gridpack or alpaka)
SCRBCK=$(basename $OUTDIR) # e.g. kokkos if $OUTDIR=epochX/kokkos

# Default output backend (in the kokkos directory this can be changed using commad line options like --cpp, --gpu or --mad)
OUTBCK=$SCRBCK
...
# Hardcoded for kokkos and alpaka: disable helicity recycling (#400, #279) for the moment
...
for arg in "$@"; do
  ...
  if [ "$arg" == "-h" ] || [ "$arg" == "--help" ]; then
    ...
    export HELREC=0
  elif [ "$arg" == "--cpp" ] && [ "${SCRBCK}" == "kokkos" ]; then
    export OUTBCK=${arg#--}
  elif [ "$arg" == "--gpu" ] && [ "${SCRBCK}" == "kokkos" ]; then
    export OUTBCK=${arg#--}
  elif [ "$arg" == "--madonly" ] && [ "${SCRBCK}" == "kokkos" ]; then
    export OUTBCK=${arg#--}
  elif [ "$arg" == "--mad" ] && [ "${SCRBCK}" == "kokkos" ]; then
    export OUTBCK=${arg#--}
  elif [ "$arg" == "--madcpp" ] && [ "${SCRBCK}" == "kokkos" ]; then
    export OUTBCK=${arg#--}
  elif [ "$arg" == "--madgpu" ] && [ "${SCRBCK}" == "kokkos" ]; then
    export OUTBCK=${arg#--}
  else
...
# Check that MG5aMC uses the git 311 branch (default for all of kokkos, alpaka, gridpack)
...
# Copy the new plugin to MG5AMC_HOME (if the script directory backend is kokkos or alpaka)
if [ "${SCRBCK}" == "kokkos" ]; then
  if [ "${OUTBCK}" == "no-path-to-this-statement" ]; then
    echo -e "\nWARNING! '${OUTBCK}' mode selected: do not copy the kokkos plugin (workaround for #341)"
  else # currently succeeds also for madcpp and madgpu (#341 has been fixed)
    echo -e "\nINFO! '${OUTBCK}' mode selected: copy the kokkos plugin\n"
    cp -dpr ${SCRDIR}/PLUGIN/${SCRBCK^^}_SA_OUTPUT ${MG5AMC_HOME}/PLUGIN/
    ls -l ${MG5AMC_HOME}/PLUGIN
  fi
elif [ "${SCRBCK}" == "alpaka" ]; then
  cp -dpr ${SCRDIR}/PLUGIN/${SCRBCK^^}_KOKKOS_SA_OUTPUT ${MG5AMC_HOME}/PLUGIN/
  ls -l ${MG5AMC_HOME}/PLUGIN
fi
