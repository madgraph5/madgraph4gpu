...
# Patch the default Fortran code to provide the integration with the kokkos plugin
# (1) Process-independent patches
\cp -dpr ${scrdir}/PLUGIN/KOKKOS_SA_OUTPUT/madgraph/iolibs/template_files/.clang-format ${dir} # new file
...
for p1dir in ${dir}/SubProcesses/P1_*; do
  cd $p1dir
  echo -e "madevent\n*madevent_kokkos" > .gitignore # new file
  ln -sf ../fbridge_common.inc . # new file
  \cp -dpr ${scrdir}/MG5aMC_patches/counters.cpp . # new file
  if [ "${dir%.mad}" == "$1" ]; then
    \cp -dpr ${scrdir}/PLUGIN/KOKKOS_SA_OUTPUT/madgraph/iolibs/template_files/gpu/timer.h . # new file, already present via kokkos in *.mad
  fi
  if [ "${nopatch}" == "0" ]; then
