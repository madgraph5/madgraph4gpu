...
int main(int argc, char **argv)
{
  ...
  auto devRnarray   = sycl::malloc_device<fptype>( nRnarray, q_ct1 ); // AOSOA[npagR][nparf][np4][neppR] (NB: nevt=npagR*neppR)
  auto devMomenta   = sycl::malloc_device<fptype>( nMomenta, q_ct1 ); // (previously was: allMomenta)
  auto devIsGoodHel = sycl::malloc_device<bool  >( ncomb, q_ct1 );
  auto devWeights   = sycl::malloc_device<fptype>( nWeights, q_ct1 ); // (previously was: meDevPtr)
  auto devMEs       = sycl::malloc_device<fptype>( nMEs, q_ct1 ); // (previously was: meDevPtr)
  auto devcHel      = sycl::malloc_device<short   >( ncomb*npar, q_ct1 );
  auto devcIPC      = sycl::malloc_device<fptype>( 6, q_ct1 );
  auto devcIPD      = sycl::malloc_device<fptype>( 2, q_ct1 );
  auto devcNGoodHel = sycl::malloc_device<int   >( 1, q_ct1 ); 
  ...
  for (int iiter = 0; iiter < niter; ++iiter)
  {
    ...
    q_ct1.submit([&](sycl::handler &cgh) {
        cgh.parallel_for(sycl::range<1>(ndim),
                [=](sycl::id<1> item) {
            size_t ievt = item.get(0);
            Proc::sigmaKin( devMomenta, devMEs, ievt, devcHel, devcIPC, devcIPD,
...
#endif
              << "Complex type               = EXTRAS::COMPLEX" << std::endl
