...
public:
  ...
  mgOnGpu::cxsmpl<double> GC_3, GC_50, GC_59;
...
namespace Parameters_sm // keep the same name rather than HardcodedParameters_sm for simplicity
{
  ...
  // Model couplings independent of aS
  constexpr mgOnGpu::cxsmpl<double> GC_3 = -( mdl_ee*mdl_complexi );
  constexpr mgOnGpu::cxsmpl<double> GC_50 = -( mdl_cw*mdl_ee*mdl_complexi )/( 2.*mdl_sw );
  constexpr mgOnGpu::cxsmpl<double> GC_59 = ( mdl_ee*mdl_complexi*mdl_sw )/( 2.*mdl_cw );
