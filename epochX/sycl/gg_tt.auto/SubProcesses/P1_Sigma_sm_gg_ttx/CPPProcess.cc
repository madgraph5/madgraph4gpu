...
namespace mg5amcGpu
{
  ...
  //ALWAYS_INLINE // attributes are not permitted in a function definition
  {
...

    {
      ...
      VVV1P0_1( w_fp[0], w_fp[1], cxmake( cIPC[0], cIPC[1] ), 0., 0., w_fp[4] );

      // Amplitude(s) for diagram number 1
      FFV1_0( w_fp[3], w_fp[2], w_fp[4], cxmake( cIPC[2], cIPC[3] ), &amp_fp[0] );
      ...
      FFV1_1( w_fp[2], w_fp[0], cxmake( cIPC[2], cIPC[3] ), cIPD[0], cIPD[1], w_fp[4] );

      // Amplitude(s) for diagram number 2
      FFV1_0( w_fp[3], w_fp[4], w_fp[1], cxmake( cIPC[2], cIPC[3] ), &amp_fp[0] );
      jamp_sv[0] -= amp_sv[0];

      // *** DIAGRAM 3 OF 3 ***

      // Wavefunction(s) for diagram number 3
      FFV1_2( w_fp[3], w_fp[0], cxmake( cIPC[2], cIPC[3] ), cIPD[0], cIPD[1], w_fp[4] );

      // Amplitude(s) for diagram number 3
      FFV1_0( w_fp[4], w_fp[2], w_fp[1], cxmake( cIPC[2], cIPC[3] ), &amp_fp[0] );
      jamp_sv[1] -= amp_sv[0];
