...
namespace Proc
{
  ...
  //ALWAYS_INLINE // attributes are not permitted in a function definition
  {
...

    {
      ...
      opzxxx( allmomenta, ievt, cHel[ihel*npar + 0], -1, w_sv[0], 0 ); // NB: opzxxx only uses pz
#else
      if ( ievt % 2 == 0 )
        opzxxx( allmomenta, ievt, cHel[ihel*npar + 0], -1, w_sv[0], 0 ); // NB: opzxxx only uses pz
      else
        oxxxxx( allmomenta, 0, ievt, cHel[ihel*npar + 0], -1, w_sv[0], 0 );
#endif

      imzxxx( allmomenta, ievt, cHel[ihel*npar + 1], +1, w_sv[1], 1 ); // NB: imzxxx only uses pz

      ixzxxx( allmomenta, ievt, cHel[ihel*npar + 2], -1, w_sv[2], 2 );

      oxzxxx( allmomenta, ievt, cHel[ihel*npar + 3], +1, w_sv[3], 3 );
