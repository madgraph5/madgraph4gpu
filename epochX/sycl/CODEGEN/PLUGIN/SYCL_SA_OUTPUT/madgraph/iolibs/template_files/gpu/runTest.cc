...
struct SYCL_TestBase : public TestDriverBase
{
  static constexpr int neppM = mgOnGpu::neppM; // AOSOA layout
  ...
  SYCL_TestBase( const std::string& refFileName ) : TestDriverBase( npar, refFileName ) {}
};

struct SYCLTest : public SYCL_TestBase
{
  ...
  // Don't remove!
  SYCLTest( const std::string& refFileName )
    : SYCL_TestBase( refFileName )
...
#define MG_INSTANTIATE_TEST_SUITE_GPU( prefix, test_suite_name )        \
  INSTANTIATE_TEST_SUITE_P( prefix,                                     \
                            test_suite_name,                            \
                            testing::Values( new SYCLTest( MG_EPOCH_REFERENCE_FILE_NAME ) ) );
