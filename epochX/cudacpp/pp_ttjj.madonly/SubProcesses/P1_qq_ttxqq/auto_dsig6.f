...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u c~ > t t~ u c~
      PD(IPROC)=U1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u d~ > t t~ u d~
      PD(IPROC)=U1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u s~ > t t~ u s~
      PD(IPROC)=U1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c u~ > t t~ c u~
      PD(IPROC)=C1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c d~ > t t~ c d~
      PD(IPROC)=C1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c s~ > t t~ c s~
      PD(IPROC)=C1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d u~ > t t~ d u~
      PD(IPROC)=D1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d c~ > t t~ d c~
      PD(IPROC)=D1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d s~ > t t~ d s~
      PD(IPROC)=D1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s u~ > t t~ s u~
      PD(IPROC)=S1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s c~ > t t~ s c~
      PD(IPROC)=S1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s d~ > t t~ s d~
      PD(IPROC)=S1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG6 = PD(0)
...
C       Select a flavor combination (need to do here for right sign)
     ...
     $     ,IVEC))
      ...
      ENDDO
      CALL SMATRIX6_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNEL,
...
C     
      ...
      SUBROUTINE SMATRIX6_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNEL,
