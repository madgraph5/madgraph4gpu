...
C     
C     Process: d d~ > e+ e- g g g WEIGHTED<=7
C     Process: d d~ > mu+ mu- g g g WEIGHTED<=7
C     Process: s s~ > e+ e- g g g WEIGHTED<=7
C     Process: s s~ > mu+ mu- g g g WEIGHTED<=7
C     Process: b b~ > e+ e- g g g WEIGHTED<=7
C     Process: b b~ > mu+ mu- g g g WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > e+ e- g g g
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > mu+ mu- g g g
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
