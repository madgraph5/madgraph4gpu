...
C     
C     Process: g d > e+ e- d s s~ WEIGHTED<=7
C     Process: g d > e+ e- d b b~ WEIGHTED<=7
C     Process: g d > mu+ mu- d s s~ WEIGHTED<=7
C     Process: g d > mu+ mu- d b b~ WEIGHTED<=7
C     Process: g s > e+ e- s d d~ WEIGHTED<=7
C     Process: g s > e+ e- s b b~ WEIGHTED<=7
C     Process: g s > mu+ mu- s d d~ WEIGHTED<=7
C     Process: g s > mu+ mu- s b b~ WEIGHTED<=7
C     Process: g b > e+ e- b d d~ WEIGHTED<=7
C     Process: g b > e+ e- b s s~ WEIGHTED<=7
C     Process: g b > mu+ mu- b d d~ WEIGHTED<=7
C     Process: g b > mu+ mu- b s s~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      IF (ABS(LPP(IB(2))).GE.1) THEN
        ...
        B2=PDG2PDF(LPP(IB(2)),5, IB(2),XBK(IB(2)), QSCALE)
      ENDIF
      ...
      IPROC=IPROC+1  ! g d > e+ e- d b b~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d > mu+ mu- d s s~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d > mu+ mu- d b b~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > e+ e- s d d~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > e+ e- s b b~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > mu+ mu- s d d~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > mu+ mu- s b b~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > e+ e- b d d~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > e+ e- b s s~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > mu+ mu- b d d~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > mu+ mu- b s s~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C     
      ...
      DATA D2,S2,B2/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0
     $ ,VECSIZE_MEMMAX*1D0/
C     ----------
C     BEGIN CODE
C     ----------
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $     ,DSQRT(ALL_Q2FACT(IB(2), IVEC)))
          B2(IVEC)=PDG2PDF(LPP(IB(2)),5, IB(2),ALL_XBK(IB(2),IVEC)
     $     ,DSQRT(ALL_Q2FACT(IB(2), IVEC)))
        ENDIF
      ENDDO
      CALL COUNTERS_STOP_COUNTER( 4 ) ! FortranPDFs=2
      ...
      IPROC=IPROC+1  ! g b > e+ e- b d d~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=G1(IVEC)*B2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
