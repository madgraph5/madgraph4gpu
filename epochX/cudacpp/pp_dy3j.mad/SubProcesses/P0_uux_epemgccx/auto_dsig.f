...
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     
C     Process: u u~ > e+ e- g c c~ WEIGHTED<=7
C     Process: u u~ > mu+ mu- g c c~ WEIGHTED<=7
C     Process: c c~ > e+ e- g u u~ WEIGHTED<=7
C     Process: c c~ > mu+ mu- g u u~ WEIGHTED<=7
C     
...
C     and update the couplings accordingly
      ...
      ENDIF
      IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! u u~ > e+ e- g c c~
C     ENDIF
...
C     IF (PASSCUTS(P1)) THEN
     ...
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT,VECSIZE_USED)  ! u u~ > e+ e- g c c~
C     ENDIF
