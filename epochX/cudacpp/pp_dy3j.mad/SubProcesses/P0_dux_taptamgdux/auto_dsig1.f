...
C     
C     Process: d u~ > ta+ ta- g d u~ WEIGHTED<=7
C     Process: d c~ > ta+ ta- g d c~ WEIGHTED<=7
C     Process: s u~ > ta+ ta- g s u~ WEIGHTED<=7
C     Process: s c~ > ta+ ta- g s c~ WEIGHTED<=7
C     Process: b u~ > ta+ ta- g b u~ WEIGHTED<=7
C     Process: b c~ > ta+ ta- g b c~ WEIGHTED<=7
C     
...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d u~ > ta+ ta- g d u~
      PD(IPROC)=D1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d c~ > ta+ ta- g d c~
      PD(IPROC)=D1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s u~ > ta+ ta- g s u~
      PD(IPROC)=S1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s c~ > ta+ ta- g s c~
      PD(IPROC)=S1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b u~ > ta+ ta- g b u~
      PD(IPROC)=B1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b c~ > ta+ ta- g b c~
      PD(IPROC)=B1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
