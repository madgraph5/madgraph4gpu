...
C     
C     Process: u u~ > ta+ ta- g d d~ WEIGHTED<=7
C     Process: u u~ > ta+ ta- g s s~ WEIGHTED<=7
C     Process: u u~ > ta+ ta- g b b~ WEIGHTED<=7
C     Process: c c~ > ta+ ta- g d d~ WEIGHTED<=7
C     Process: c c~ > ta+ ta- g s s~ WEIGHTED<=7
C     Process: c c~ > ta+ ta- g b b~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u~ > ta+ ta- g d d~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u u~ > ta+ ta- g s s~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u u~ > ta+ ta- g b b~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > ta+ ta- g d d~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > ta+ ta- g s s~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > ta+ ta- g b b~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
