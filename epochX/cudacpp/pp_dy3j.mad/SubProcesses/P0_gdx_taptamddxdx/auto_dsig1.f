...
C     
C     Process: g d~ > ta+ ta- d d~ d~ WEIGHTED<=7
C     Process: g s~ > ta+ ta- s s~ s~ WEIGHTED<=7
C     Process: g b~ > ta+ ta- b b~ b~ WEIGHTED<=7
C     
...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g d~ > ta+ ta- d d~ d~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > ta+ ta- s s~ s~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > ta+ ta- b b~ b~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
