...
C     
C     Process: g d~ > e+ e- g g d~ WEIGHTED<=7
C     Process: g d~ > mu+ mu- g g d~ WEIGHTED<=7
C     Process: g s~ > e+ e- g g s~ WEIGHTED<=7
C     Process: g s~ > mu+ mu- g g s~ WEIGHTED<=7
C     Process: g b~ > e+ e- g g b~ WEIGHTED<=7
C     Process: g b~ > mu+ mu- g g b~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > e+ e- g g b~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > mu+ mu- g g b~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
