...
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     
C     Process: g u > e+ e- u c c~ WEIGHTED<=7
C     Process: g u > mu+ mu- u c c~ WEIGHTED<=7
C     Process: g c > e+ e- c u u~ WEIGHTED<=7
C     Process: g c > mu+ mu- c u u~ WEIGHTED<=7
C     
...
C     and update the couplings accordingly
      ...
      ENDIF
      IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! g u > e+ e- u c c~
C     ENDIF
...
C     IF (PASSCUTS(P1)) THEN
     ...
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT,VECSIZE_USED)  ! g u > e+ e- u c c~
C     ENDIF
