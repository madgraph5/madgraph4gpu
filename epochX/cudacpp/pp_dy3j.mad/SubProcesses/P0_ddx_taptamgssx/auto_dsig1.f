...
C     
C     Process: d d~ > ta+ ta- g s s~ WEIGHTED<=7
C     Process: d d~ > ta+ ta- g b b~ WEIGHTED<=7
C     Process: s s~ > ta+ ta- g d d~ WEIGHTED<=7
C     Process: s s~ > ta+ ta- g b b~ WEIGHTED<=7
C     Process: b b~ > ta+ ta- g d d~ WEIGHTED<=7
C     Process: b b~ > ta+ ta- g s s~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d d~ > ta+ ta- g s s~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > ta+ ta- g b b~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > ta+ ta- g d d~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > ta+ ta- g b b~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > ta+ ta- g d d~
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > ta+ ta- g s s~
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
