...
C     
C     Process: d s~ > e+ e- g d s~ WEIGHTED<=7
C     Process: d s~ > mu+ mu- g d s~ WEIGHTED<=7
C     Process: d b~ > e+ e- g d b~ WEIGHTED<=7
C     Process: d b~ > mu+ mu- g d b~ WEIGHTED<=7
C     Process: s d~ > e+ e- g s d~ WEIGHTED<=7
C     Process: s d~ > mu+ mu- g s d~ WEIGHTED<=7
C     Process: s b~ > e+ e- g s b~ WEIGHTED<=7
C     Process: s b~ > mu+ mu- g s b~ WEIGHTED<=7
C     Process: b d~ > e+ e- g b d~ WEIGHTED<=7
C     Process: b d~ > mu+ mu- g b d~ WEIGHTED<=7
C     Process: b s~ > e+ e- g b s~ WEIGHTED<=7
C     Process: b s~ > mu+ mu- g b s~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      IF (ABS(LPP(IB(2))).GE.1) THEN
        ...
        ENDIF
        BX2=PDG2PDF(LPP(IB(2)),-5, IB(2),XBK(IB(2)), QSCALE)
      ...
      PD(IPROC)=D1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d b~ > e+ e- g d b~
      PD(IPROC)=D1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d b~ > mu+ mu- g d b~
      PD(IPROC)=D1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      ...
      PD(IPROC)=S1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s b~ > e+ e- g s b~
      PD(IPROC)=S1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s b~ > mu+ mu- g s b~
      PD(IPROC)=S1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b d~ > e+ e- g b d~
      PD(IPROC)=B1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b d~ > mu+ mu- g b d~
      PD(IPROC)=B1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b s~ > e+ e- g b s~
      PD(IPROC)=B1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b s~ > mu+ mu- g b s~
      PD(IPROC)=B1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C     
     ...
     $ ,B1(VECSIZE_MEMMAX)
      DOUBLE PRECISION BX2(VECSIZE_MEMMAX),SX2(VECSIZE_MEMMAX)
     $ ,DX2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     DATA
C     
      DATA D1,S1,B1/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0
     $ ,VECSIZE_MEMMAX*1D0/
      DATA BX2,SX2,DX2/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0
     $ ,VECSIZE_MEMMAX*1D0/
C     ----------
C     BEGIN CODE
C     ----------
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $     ,DSQRT(ALL_Q2FACT(IB(1), IVEC)))
        ...
        IF (ABS(LPP(IB(2))).GE.1) THEN
            !LP=SIGN(1,LPP(IB(2)))
          BX2(IVEC)=PDG2PDF(LPP(IB(2)),-5, IB(2),ALL_XBK(IB(2),IVEC)
     ...
     $     ,DSQRT(ALL_Q2FACT(IB(2), IVEC)))
      ...
      IPROC=IPROC+1  ! d b~ > e+ e- g d b~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=D1(IVEC)*BX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
      ...
      IPROC=IPROC+1  ! s b~ > e+ e- g s b~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=S1(IVEC)*BX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
      ...
      IPROC=IPROC+1  ! b d~ > e+ e- g b d~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=B1(IVEC)*DX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
      ...
      IPROC=IPROC+1  ! b s~ > e+ e- g b s~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=B1(IVEC)*SX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
