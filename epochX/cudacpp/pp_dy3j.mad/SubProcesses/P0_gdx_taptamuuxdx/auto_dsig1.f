...
C     
C     Process: g d~ > ta+ ta- u u~ d~ WEIGHTED<=7
C     Process: g d~ > ta+ ta- c c~ d~ WEIGHTED<=7
C     Process: g s~ > ta+ ta- u u~ s~ WEIGHTED<=7
C     Process: g s~ > ta+ ta- c c~ s~ WEIGHTED<=7
C     Process: g b~ > ta+ ta- u u~ b~ WEIGHTED<=7
C     Process: g b~ > ta+ ta- c c~ b~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g d~ > ta+ ta- u u~ d~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d~ > ta+ ta- c c~ d~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > ta+ ta- u u~ s~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > ta+ ta- c c~ s~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > ta+ ta- u u~ b~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > ta+ ta- c c~ b~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
