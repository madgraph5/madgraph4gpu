...
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     
C     Process: g u~ > ta+ ta- c u~ c~ WEIGHTED<=7
C     Process: g c~ > ta+ ta- u c~ u~ WEIGHTED<=7
C     
...
C     and update the couplings accordingly
      ...
      ENDIF
      IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! g u~ > ta+ ta- c u~ c~
C     ENDIF
...
C     IF (PASSCUTS(P1)) THEN
     ...
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT,VECSIZE_USED)  ! g u~ > ta+ ta- c u~ c~
C     ENDIF
