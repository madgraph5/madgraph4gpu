...
C     
C     Process: d~ d~ > e+ e- g d~ d~ WEIGHTED<=7
C     Process: d~ d~ > mu+ mu- g d~ d~ WEIGHTED<=7
C     Process: s~ s~ > e+ e- g s~ s~ WEIGHTED<=7
C     Process: s~ s~ > mu+ mu- g s~ s~ WEIGHTED<=7
C     Process: b~ b~ > e+ e- g b~ b~ WEIGHTED<=7
C     Process: b~ b~ > mu+ mu- g b~ b~ WEIGHTED<=7
C     
...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b~ b~ > e+ e- g b~ b~
      PD(IPROC)=BX1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b~ b~ > mu+ mu- g b~ b~
      PD(IPROC)=BX1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C     
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION QSCALE
      DOUBLE PRECISION BX1(VECSIZE_MEMMAX),SX1(VECSIZE_MEMMAX)
     $ ,DX1(VECSIZE_MEMMAX)
...
C     DATA
C     
      DATA BX1,SX1,DX1/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0
     $ ,VECSIZE_MEMMAX*1D0/
...
C           LP=SIGN(1,LPP(IB(2)))
     ...
     $       ,DSQRT(ALL_Q2FACT(IB(2), IVEC)))
      ...
      IPROC=IPROC+1  ! b~ b~ > e+ e- g b~ b~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=BX1(IVEC)*BX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
