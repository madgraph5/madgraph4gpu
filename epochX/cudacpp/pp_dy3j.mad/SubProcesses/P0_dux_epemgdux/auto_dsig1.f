...
C     
C     Process: d u~ > e+ e- g d u~ WEIGHTED<=7
C     Process: d u~ > mu+ mu- g d u~ WEIGHTED<=7
C     Process: d c~ > e+ e- g d c~ WEIGHTED<=7
C     Process: d c~ > mu+ mu- g d c~ WEIGHTED<=7
C     Process: s u~ > e+ e- g s u~ WEIGHTED<=7
C     Process: s u~ > mu+ mu- g s u~ WEIGHTED<=7
C     Process: s c~ > e+ e- g s c~ WEIGHTED<=7
C     Process: s c~ > mu+ mu- g s c~ WEIGHTED<=7
C     Process: b u~ > e+ e- g b u~ WEIGHTED<=7
C     Process: b u~ > mu+ mu- g b u~ WEIGHTED<=7
C     Process: b c~ > e+ e- g b c~ WEIGHTED<=7
C     Process: b c~ > mu+ mu- g b c~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(IPROC)=S1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b u~ > e+ e- g b u~
      PD(IPROC)=B1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b u~ > mu+ mu- g b u~
      PD(IPROC)=B1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b c~ > e+ e- g b c~
      PD(IPROC)=B1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b c~ > mu+ mu- g b c~
      PD(IPROC)=B1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $     ,DSQRT(ALL_Q2FACT(IB(2), IVEC)))
      ...
      IPROC=IPROC+1  ! b u~ > e+ e- g b u~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=B1(IVEC)*UX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
      ...
      IPROC=IPROC+1  ! b c~ > e+ e- g b c~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=B1(IVEC)*CX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
