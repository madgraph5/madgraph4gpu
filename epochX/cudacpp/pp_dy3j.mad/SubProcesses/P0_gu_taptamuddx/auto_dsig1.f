...
C     
C     Process: g u > ta+ ta- u d d~ WEIGHTED<=7
C     Process: g u > ta+ ta- u s s~ WEIGHTED<=7
C     Process: g u > ta+ ta- u b b~ WEIGHTED<=7
C     Process: g c > ta+ ta- c d d~ WEIGHTED<=7
C     Process: g c > ta+ ta- c s s~ WEIGHTED<=7
C     Process: g c > ta+ ta- c b b~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g u > ta+ ta- u d d~
      PD(IPROC)=G1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g u > ta+ ta- u s s~
      PD(IPROC)=G1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g u > ta+ ta- u b b~
      PD(IPROC)=G1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c > ta+ ta- c d d~
      PD(IPROC)=G1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c > ta+ ta- c s s~
      PD(IPROC)=G1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c > ta+ ta- c b b~
      PD(IPROC)=G1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
