...
C     
C     Process: u d~ > ta+ ta- g u d~ WEIGHTED<=7
C     Process: u s~ > ta+ ta- g u s~ WEIGHTED<=7
C     Process: u b~ > ta+ ta- g u b~ WEIGHTED<=7
C     Process: c d~ > ta+ ta- g c d~ WEIGHTED<=7
C     Process: c s~ > ta+ ta- g c s~ WEIGHTED<=7
C     Process: c b~ > ta+ ta- g c b~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u d~ > ta+ ta- g u d~
      PD(IPROC)=U1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u s~ > ta+ ta- g u s~
      PD(IPROC)=U1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u b~ > ta+ ta- g u b~
      PD(IPROC)=U1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c d~ > ta+ ta- g c d~
      PD(IPROC)=C1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c s~ > ta+ ta- g c s~
      PD(IPROC)=C1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c b~ > ta+ ta- g c b~
      PD(IPROC)=C1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
