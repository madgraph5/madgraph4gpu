...
C     
C     Process: g d > ta+ ta- u d u~ WEIGHTED<=7
C     Process: g d > ta+ ta- c d c~ WEIGHTED<=7
C     Process: g s > ta+ ta- u s u~ WEIGHTED<=7
C     Process: g s > ta+ ta- c s c~ WEIGHTED<=7
C     Process: g b > ta+ ta- u b u~ WEIGHTED<=7
C     Process: g b > ta+ ta- c b c~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g d > ta+ ta- u d u~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d > ta+ ta- c d c~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > ta+ ta- u s u~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > ta+ ta- c s c~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > ta+ ta- u b u~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > ta+ ta- c b c~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
