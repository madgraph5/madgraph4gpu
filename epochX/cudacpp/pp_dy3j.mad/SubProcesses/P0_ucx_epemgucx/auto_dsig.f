...
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     
C     Process: u c~ > e+ e- g u c~ WEIGHTED<=7
C     Process: u c~ > mu+ mu- g u c~ WEIGHTED<=7
C     Process: c u~ > e+ e- g c u~ WEIGHTED<=7
C     Process: c u~ > mu+ mu- g c u~ WEIGHTED<=7
C     
...
C     and update the couplings accordingly
      ...
      ENDIF
      IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! u c~ > e+ e- g u c~
C     ENDIF
...
C     IF (PASSCUTS(P1)) THEN
     ...
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT,VECSIZE_USED)  ! u c~ > e+ e- g u c~
C     ENDIF
