...
C     
C     Process: g d~ > e+ e- s d~ s~ WEIGHTED<=7
C     Process: g d~ > e+ e- b d~ b~ WEIGHTED<=7
C     Process: g d~ > mu+ mu- s d~ s~ WEIGHTED<=7
C     Process: g d~ > mu+ mu- b d~ b~ WEIGHTED<=7
C     Process: g s~ > e+ e- d s~ d~ WEIGHTED<=7
C     Process: g s~ > e+ e- b s~ b~ WEIGHTED<=7
C     Process: g s~ > mu+ mu- d s~ d~ WEIGHTED<=7
C     Process: g s~ > mu+ mu- b s~ b~ WEIGHTED<=7
C     Process: g b~ > e+ e- d b~ d~ WEIGHTED<=7
C     Process: g b~ > e+ e- s b~ s~ WEIGHTED<=7
C     Process: g b~ > mu+ mu- d b~ d~ WEIGHTED<=7
C     Process: g b~ > mu+ mu- s b~ s~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      IPROC=IPROC+1  ! g d~ > e+ e- b d~ b~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d~ > mu+ mu- s d~ s~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d~ > mu+ mu- b d~ b~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > e+ e- d s~ d~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > e+ e- b s~ b~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > mu+ mu- d s~ d~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > mu+ mu- b s~ b~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > e+ e- d b~ d~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > e+ e- s b~ s~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > mu+ mu- d b~ d~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > mu+ mu- s b~ s~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $     ,DSQRT(ALL_Q2FACT(IB(2), IVEC)))
      ...
      IPROC=IPROC+1  ! g b~ > e+ e- d b~ d~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=G1(IVEC)*BX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
