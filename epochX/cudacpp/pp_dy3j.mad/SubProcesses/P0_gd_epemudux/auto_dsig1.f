...
C     
C     Process: g d > e+ e- u d u~ WEIGHTED<=7
C     Process: g d > e+ e- c d c~ WEIGHTED<=7
C     Process: g d > mu+ mu- u d u~ WEIGHTED<=7
C     Process: g d > mu+ mu- c d c~ WEIGHTED<=7
C     Process: g s > e+ e- u s u~ WEIGHTED<=7
C     Process: g s > e+ e- c s c~ WEIGHTED<=7
C     Process: g s > mu+ mu- u s u~ WEIGHTED<=7
C     Process: g s > mu+ mu- c s c~ WEIGHTED<=7
C     Process: g b > e+ e- u b u~ WEIGHTED<=7
C     Process: g b > e+ e- c b c~ WEIGHTED<=7
C     Process: g b > mu+ mu- u b u~ WEIGHTED<=7
C     Process: g b > mu+ mu- c b c~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > e+ e- u b u~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > e+ e- c b c~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > mu+ mu- u b u~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > mu+ mu- c b c~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
