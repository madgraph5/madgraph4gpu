...
C     
C     Process: g d~ > e+ e- d d~ d~ WEIGHTED<=7
C     Process: g d~ > mu+ mu- d d~ d~ WEIGHTED<=7
C     Process: g s~ > e+ e- s s~ s~ WEIGHTED<=7
C     Process: g s~ > mu+ mu- s s~ s~ WEIGHTED<=7
C     Process: g b~ > e+ e- b b~ b~ WEIGHTED<=7
C     Process: g b~ > mu+ mu- b b~ b~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > e+ e- b b~ b~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > mu+ mu- b b~ b~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
