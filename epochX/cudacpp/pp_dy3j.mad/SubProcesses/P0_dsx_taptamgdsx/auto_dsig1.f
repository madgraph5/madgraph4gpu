...
C     
C     Process: d s~ > ta+ ta- g d s~ WEIGHTED<=7
C     Process: d b~ > ta+ ta- g d b~ WEIGHTED<=7
C     Process: s d~ > ta+ ta- g s d~ WEIGHTED<=7
C     Process: s b~ > ta+ ta- g s b~ WEIGHTED<=7
C     Process: b d~ > ta+ ta- g b d~ WEIGHTED<=7
C     Process: b s~ > ta+ ta- g b s~ WEIGHTED<=7
C     
...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d s~ > ta+ ta- g d s~
      PD(IPROC)=D1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d b~ > ta+ ta- g d b~
      PD(IPROC)=D1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s d~ > ta+ ta- g s d~
      PD(IPROC)=S1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s b~ > ta+ ta- g s b~
      PD(IPROC)=S1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b d~ > ta+ ta- g b d~
      PD(IPROC)=B1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b s~ > ta+ ta- g b s~
      PD(IPROC)=B1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
