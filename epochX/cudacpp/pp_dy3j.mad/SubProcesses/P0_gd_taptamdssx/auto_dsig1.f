...
C     
C     Process: g d > ta+ ta- d s s~ WEIGHTED<=7
C     Process: g d > ta+ ta- d b b~ WEIGHTED<=7
C     Process: g s > ta+ ta- s d d~ WEIGHTED<=7
C     Process: g s > ta+ ta- s b b~ WEIGHTED<=7
C     Process: g b > ta+ ta- b d d~ WEIGHTED<=7
C     Process: g b > ta+ ta- b s s~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g d > ta+ ta- d s s~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d > ta+ ta- d b b~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > ta+ ta- s d d~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > ta+ ta- s b b~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > ta+ ta- b d d~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b > ta+ ta- b s s~
      PD(IPROC)=G1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
