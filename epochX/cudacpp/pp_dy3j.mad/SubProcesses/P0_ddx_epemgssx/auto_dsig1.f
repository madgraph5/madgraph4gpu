...
C     
C     Process: d d~ > e+ e- g s s~ WEIGHTED<=7
C     Process: d d~ > e+ e- g b b~ WEIGHTED<=7
C     Process: d d~ > mu+ mu- g s s~ WEIGHTED<=7
C     Process: d d~ > mu+ mu- g b b~ WEIGHTED<=7
C     Process: s s~ > e+ e- g d d~ WEIGHTED<=7
C     Process: s s~ > e+ e- g b b~ WEIGHTED<=7
C     Process: s s~ > mu+ mu- g d d~ WEIGHTED<=7
C     Process: s s~ > mu+ mu- g b b~ WEIGHTED<=7
C     Process: b b~ > e+ e- g d d~ WEIGHTED<=7
C     Process: b b~ > e+ e- g s s~ WEIGHTED<=7
C     Process: b b~ > mu+ mu- g d d~ WEIGHTED<=7
C     Process: b b~ > mu+ mu- g s s~ WEIGHTED<=7
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      IPROC=IPROC+1  ! d d~ > e+ e- g b b~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > mu+ mu- g s s~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > mu+ mu- g b b~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > e+ e- g d d~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > e+ e- g b b~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > mu+ mu- g d d~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > mu+ mu- g b b~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > e+ e- g d d~
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > e+ e- g s s~
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > mu+ mu- g d d~
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b b~ > mu+ mu- g s s~
      PD(IPROC)=B1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $     ,DSQRT(ALL_Q2FACT(IB(2), IVEC)))
      ...
      IPROC=IPROC+1  ! b b~ > e+ e- g d d~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=B1(IVEC)*BX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
