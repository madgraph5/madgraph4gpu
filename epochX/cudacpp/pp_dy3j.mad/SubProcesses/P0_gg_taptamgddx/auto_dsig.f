...
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     
C     Process: g g > ta+ ta- g d d~ WEIGHTED<=7
C     Process: g g > ta+ ta- g s s~ WEIGHTED<=7
C     Process: g g > ta+ ta- g b b~ WEIGHTED<=7
C     
...
C     and update the couplings accordingly
      ...
      ENDIF
      IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! g g > ta+ ta- g d d~
C     ENDIF
...
C     IF (PASSCUTS(P1)) THEN
     ...
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT,VECSIZE_USED)  ! g g > ta+ ta- g d d~
C     ENDIF
