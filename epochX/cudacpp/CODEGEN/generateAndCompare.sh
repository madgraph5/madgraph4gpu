...
function codeGenAndDiff()
{
  ...
  if [ "${SCRBCK}" == "gridpack" ] && [ "${UNTARONLY}" == "1" ]; then
    ...
    elif [ "${OUTBCK}" == "mad2" ]; then # $SCRBCK=cudacpp and $OUTBCK=mad
      echo "output ${outproc} ${helrecopt} --vector_size=${vecsize} --me_exporter=standalone_cudacpp" >> ${outproc}.mg
  ...
  elif [ "${OUTBCK}" == "madnovec" ] || [ "${OUTBCK}" == "madonly" ] || [ "${OUTBCK}" == "mad" ] || [ "${OUTBCK}" == "mad2" ] || [ "${OUTBCK}" == "madcpp" ] || [ "${OUTBCK}" == "madgpu" ]; then
    autosuffix=${OUTBCK}
  fi
  ...
  if [ "${OUTBCK}" == "madnovec" ] || [ "${OUTBCK}" == "madonly" ] || [ "${OUTBCK}" == "mad" ] || [ "${OUTBCK}" == "mad2" ] || [ "${OUTBCK}" == "madcpp" ] || [ "${OUTBCK}" == "madgpu" ]; then
  ...
  if [ "${OUTBCK}" == "mad" ] || [ "${OUTBCK}" == "mad2" ]; then
...
function usage()
{
  ...
  if [ "${SCRBCK}" == "gridpack" ]; then
    ...
    echo "Usage:   $0 [--nobrief] [--cpp|--gpu|--madnovec|--madonly|--mad|--mad2|--madcpp*|--madgpu] [--nopatch|--upstream] [-c '<cmd>'] <proc>"
...
while [ "$1" != "" ]; do
  if [ "$1" == "-h" ] || [ "$1" == "--help" ]; then
    ...
    export OUTBCK=${1#--}
  elif [ "$1" == "--mad2" ] && [ "${SCRBCK}" == "cudacpp" ]; then
    export OUTBCK=${1#--}
...
elif [ "${OUTBCK}" == "mad" ] || [ "${OUTBCK}" == "mad2" ]; then
