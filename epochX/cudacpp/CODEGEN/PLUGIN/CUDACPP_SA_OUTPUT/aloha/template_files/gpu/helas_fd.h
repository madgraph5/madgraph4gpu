...
//==========================================================================
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    if( vmass != 0. )
    {
      ...
      if ( std::abs(nhel) == 1.)
      {
        ...
        vc[2] = cxmake( -vmass/nk * n[0]);
        vc[3] = cxmake( -vmass/nk * n[1]);
        vc[4] = cxmake( -vmass/nk * n[2]);
        vc[5] = cxmake( -vmass/nk * n[3]);
        vc[6] = cxmake( -nsv*cI);
      }
      ...
      //FD gauge
      //branch A
      nA[0] = fpternary( pvec0 >= 0. , 1. , -1.);
      nA[1] = -p[1]/pp;
      nA[2] = -p[2]/pp;
      nA[3] = -p[3]/pp;
      nA[4] = static_cast<fptype_sv>(0.);

      //branch B
      nB[0] = nA[0];
      nB[1] = static_cast<fptype_sv>(0.);
      nB[2] = static_cast<fptype_sv>(0.);
      nB[3] = -nA[0];

      const fptype_sv b_A = fpternary(pp > 0., 1. , 0.);
      const fptype_sv b_B = fpternary(pp <= 0., 1. , 0.);
      ...
      vc[2] = cxternary( mask3, vc[2], cxmake( -vmass/nk n[0]));
      vc[3] = cxternary( mask3, vc[3], cxmake( -vmass/nk n[1]));
      vc[4] = cxternary( mask3, vc[4], cxmake( -vmass/nk n[2]));
      vc[5] = cxternary( mask3, vc[5], cxmake( -vmass/nk n[3]));
      vc[6] = cxternary( mask3, cxmake(-nsv*cI), cxzero())
#endif
    }
    else
    {
 ...
 {
   const fptype_sv gabs2 = q[1]*q[1] + q[2]*q[2] + q[3]*q[3];
#ifndef MGONGPU_CPPSIMD
...
// Compute propagator factor n[5] of the gauge q[5]
  ...
  calculate_propagator_factor(const cxtype_sv q[5],
                                 ...
                                 fptype_sv *d)
  {
    const fptype_sv  q2 = q[0]*q[0] - ( q[1]*q[1] + q[2]*q[2] + q[3]*q[3] );
    *d = 1.0 / (q2 - mass*mass);
  }
//--------------------------------------------------------------------------
// multiply by propagation factor from m and wawefunctionsin[] and output them
