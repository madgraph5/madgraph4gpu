...
namespace mg5amcCpu
{
  ...
  void BridgeKernelHost::computeMatrixElements( const bool useChannelIds )
  {
    ...
    m_bridge.cpu_sequence( m_fortranMomenta.data(), m_gs.data(), m_rndhel.data(), m_rndcol.data(), pChannelIds, m_igraph1, m_matrixElements.data(), m_selhel.data(), m_selcol.data(), goodHelOnly );
  }
...
namespace mg5amcGpu
{
  ...
  void BridgeKernelDevice::computeMatrixElements( const bool useChannelIds )
  {
    constexpr bool goodHelOnly = false;
    const unsigned int* pChannelIds = ( useChannelIds ? m_channelIds.data() : nullptr );
    m_bridge.gpu_sequence( m_fortranMomenta.data(), m_gs.data(), m_rndhel.data(), m_rndcol.data(), pChannelIds, m_igraph1, m_matrixElements.data(), m_selhel.data(), m_selcol.data(), goodHelOnly );
  }
