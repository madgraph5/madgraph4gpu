...
class PLUGIN_OneProcessExporter(export_cpp.OneProcessExporterGPU):
    ...
    def get_process_function_definitions(self, write=True):
        ...
        for coup in coupling : coup_str_hrd += "    HardcodedParameters_sm::%s.real(), HardcodedParameters_sm::%s.imag(),\n" % ( coup, coup )
        coup_str_hrd = coup_str_hrd[:-2] + " };\n"
        param_str_hrd = "  __device__ const fptype cIPD[%s] = {\n" % len(self.params2order)
        for para in params : param_str_hrd += "    HardcodedParameters_sm::%s,\n" % para
