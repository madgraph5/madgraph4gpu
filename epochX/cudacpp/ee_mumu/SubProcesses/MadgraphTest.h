...
namespace
{
  ...
  std::map<unsigned int, ReferenceData> readReferenceData( const std::string& refFileName )
  {
    ...
    for ( std::string line; std::getline( referenceFile, line ); )
    {
      ...
      {
        if ( evtNo <= referenceData[batchNo].MEs.size() ) referenceData[batchNo].MEs.resize( evtNo + 1 );
      ...
      else
      {
        ...
        if ( evtNo <= referenceData[batchNo].momenta.size() ) referenceData[batchNo].momenta.resize( evtNo + 1 );
...
public:
  ...
  TestDriverBase( unsigned int npart = mgOnGpu::npar,
                  const std::string& refFileName = "" ) // default value "" is for backward compatibility
    : m_refFileName( refFileName != "" ? refFileName
                                       : std::string( "../../../../../test/ref/dump_CPUTest.Sigma_sm_epem_mupmum.txt" ) )
    , nparticle( npart )
  {
  }
  ...
  void dumpParticles( std::ostream& stream,
                      std::size_t ievt,
                      ...
                      const ReferenceData& referenceData ) const
  {
    ...
    for ( unsigned int ipar = 0; ipar < numParticles; ipar++ )
    {
      ...
      stream << std::scientific // fixed format: affects all floats (default nDigit: 6)
             << std::setprecision( nDigit ) << std::setw( 4 ) << ipar << std::setw( width ) << getMomentum( ievt, ipar, 0 )
             << std::setw( width ) << getMomentum( ievt, ipar, 1 ) << std::setw( width ) << getMomentum( ievt, ipar, 2 )
             << std::setw( width ) << getMomentum( ievt, ipar, 3 ) << "\n";
      if ( ievt < referenceData.momenta.size() )
      {
        stream << "ref" << ipar;
        stream << std::setw( width ) << referenceData.momenta[ievt][ipar][0] << std::setw( width )
               << referenceData.momenta[ievt][ipar][1] << std::setw( width ) << referenceData.momenta[ievt][ipar][2]
...
protected:
  ...
  MadgraphTest() : TestWithParam(), testDriver( GetParam() ) {}
};
...
TEST_P( MadgraphTest, CompareMomentaAndME )
{
  ...
  std::string dumpFileName = std::string( "dump_" ) + testing::UnitTest::GetInstance()->current_test_info()->test_suite_name() +
    '.' + testing::UnitTest::GetInstance()->current_test_info()->name() + ".txt";
  while ( dumpFileName.find( '/' ) != std::string::npos ) { dumpFileName.replace( dumpFileName.find( '/' ), 1, "_" ); }
  const std::string refFileName = testDriver->getRefFileName();
  std::ofstream dumpFile;
  if ( dumpEvents ) { dumpFile.open( dumpFileName, std::ios::trunc ); }
  // Read reference data
  std::map<unsigned int, ReferenceData> referenceData;
  if ( !dumpEvents ) { referenceData = readReferenceData( refFileName ); }
  ...
  for ( unsigned int iiter = 0; iiter < testDriver->niter; ++iiter )
  {
    ...
    for ( std::size_t ievt = 0; ievt < testDriver->nevt && !HasFailure(); ++ievt )
    {
      if ( dumpEvents )
      {
        ...
        // Dump matrix element
        dumpFile << std::setw( 4 ) << "ME" << std::scientific << std::setw( 15 + 8 ) << testDriver->getMatrixElement( ievt )
                 << "\n"
                 << std::endl
                 << std::defaultfloat;
        continue;
      ...
      eventTrace << std::setw( 4 ) << "ME" << std::scientific << std::setw( 15 + 8 ) << testDriver->getMatrixElement( ievt )
                 << "\n"
                 << std::setw( 4 ) << "r.ME" << std::scientific << std::setw( 15 + 8 ) << referenceData[iiter].MEs[ievt]
                 << std::endl
                 << std::defaultfloat;
      ...
      // Compare ME:
      EXPECT_NEAR(
        testDriver->getMatrixElement( ievt ), referenceData[iiter].MEs[ievt], toleranceMEs * referenceData[iiter].MEs[ievt] );
    }
  }
  if ( dumpEvents ) { std::cout << "Event dump written to " << dumpFileName << std::endl; }
}
