...
private:
  ...
  static __host__ __device__ inline fptype& decodeRecord( fptype* buffer, const int ip4, const int ipar )
  {
...
public:
  ...
  */
  ...
  static __host__ __device__ inline fptype_sv kernelAccessIp4IparConst( const fptype* buffer, const int ip4, const int ipar )
  {
    ...
    if constexpr ( useContiguousEventsIfPossible && ( neppM >= neppV ) && ( neppM % neppV == 0 ) )
    {
      ...
      else
      {
        ...
        return mg5amcCpu::fptypevFromUnalignedArray(
          out ); // SIMD bulk load of neppV, do not use reinterpret_cast (fewer SIMD operations)
      }
    }
    else
    {
      ...
      constexpr int ievt0 =
        0; // just make it explicit in the code that buffer refers to a given ievt0 and decoderIeppV fetches event ievt0+ieppV
  ...
  static __host__ __device__ inline constexpr bool isOnDevice() { return onDevice; }
};
