...
private:
  ...
  static __host__ __device__ inline fptype* ieventAccessRecord( fptype* buffer, const int ievt )
  {
    ...
    return &(
      buffer[ipagR * nparf * np4 * neppR + iparf * np4 * neppR + ip4 * neppR + ieppR] ); // AOSOA[ipagR][iparf][ip4][ieppR]
  }
  ...
  static __host__ __device__ inline fptype& decodeRecord( fptype* buffer, const int ip4, const int iparf )
  {
