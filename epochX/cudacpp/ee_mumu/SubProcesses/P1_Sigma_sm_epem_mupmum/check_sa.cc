...
int main( int argc, char **argv )
{
  ...
  if ( rndgen == RandomNumberMode::CommonRandom ) { prnk.reset( new CommonRandomNumberKernel( hstRnarray ) ); }
#ifndef MGONGPU_HAS_NO_CURAND
      ...
      if ( verbose )
      {
        ...
        for ( int ipar = 0; ipar < mgOnGpu::npar; ipar++ )
        {
          ...
          std::cout << std::scientific // fixed format: affects all floats (default precision: 6)
                    << std::setw( 4 ) << ipar + 1 << std::setw( 14 )
                    << MemoryAccessMomenta::ieventAccessIp4IparConst( hstMomenta.data(), ievt, 0, ipar ) << std::setw( 14 )
                    << MemoryAccessMomenta::ieventAccessIp4IparConst( hstMomenta.data(), ievt, 1, ipar ) << std::setw( 14 )
                    << MemoryAccessMomenta::ieventAccessIp4IparConst( hstMomenta.data(), ievt, 2, ipar ) << std::setw( 14 )
                    << MemoryAccessMomenta::ieventAccessIp4IparConst( hstMomenta.data(), ievt, 3, ipar ) << std::endl
...
#error Internal error: unknown SIMD build configuration
              ...
              << std::defaultfloat; // default format: affects all floats
    std::cout << std::string( SEP79, '*' ) << std::endl << hstStats;
  }
