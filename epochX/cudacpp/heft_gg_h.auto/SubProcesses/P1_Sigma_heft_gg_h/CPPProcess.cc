...
#endif
{
  ...
  __device__ const fptype cIPC[2] = {
    Parameters_sm::GC_13.real(), Parameters_sm::GC_13.imag() };
#else
#ifdef __CUDACC__
...
#ifdef MGONGPU_CPPSIMD
#pragma omp parallel for default(none) shared(allmomenta,allMEs,cHel,cIPC,ihel,npagV,amp_fp,w_fp,isAligned_allMEs) private (amp_sv,w_sv,jamp_sv)
#else
#pragma omp parallel for default(none) shared(allmomenta,allMEs,cHel,cIPC,ihel,npagV,amp_fp,w_fp) private (amp_sv,w_sv,jamp_sv)
#endif
#endif
...
#endif
    {
      ...
      sxxxxx<M_ACCESS, W_ACCESS>( momenta, m_pars->mdl_MH, +1, w_fp[2], 2 );
      ...
      static constexpr fptype cf[ncolor][ncolor] = {{2}};
