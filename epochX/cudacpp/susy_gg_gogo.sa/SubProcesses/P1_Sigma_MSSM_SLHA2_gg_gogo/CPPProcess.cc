...
// Process: g g > go go WEIGHTED<=2 @1
...
#endif
{
  ...
  __device__ const fptype cIPD[2] = { (fptype)Parameters_MSSM_SLHA2::mdl_Mgo, (fptype)Parameters_MSSM_SLHA2::mdl_Wgo };
...
#pragma nv_diagnostic pop
      ...
      FFV1_0<W_ACCESS, A_ACCESS, CD_ACCESS>( w_fp[2], w_fp[3], w_fp[4], COUPs[0], -1.0, &amp_fp[0] );
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
      // Here the code base generated with multichannel support updates numerators_sv and denominators_sv (#473)
#endif
      jamp_sv[0] += 2. * amp_sv[0];
      jamp_sv[1] -= 2. * amp_sv[0];
      jamp_sv[3] -= 2. * amp_sv[0];
      jamp_sv[5] += 2. * amp_sv[0];

      // *** DIAGRAM 2 OF 3 ***

      // Wavefunction(s) for diagram number 2
      FFV1_2<W_ACCESS, CD_ACCESS>( w_fp[2], w_fp[0], COUPs[0], 1.0, cIPD[0], cIPD[1], w_fp[4] );

      // Amplitude(s) for diagram number 2
      FFV1_0<W_ACCESS, A_ACCESS, CD_ACCESS>( w_fp[4], w_fp[3], w_fp[1], COUPs[0], -1.0, &amp_fp[0] );
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
      // Here the code base generated with multichannel support updates numerators_sv and denominators_sv (#473)
#endif
      jamp_sv[1] += 2. * amp_sv[0];
      ...
      jamp_sv[3] += 2. * amp_sv[0];
      jamp_sv[4] -= 2. * amp_sv[0];

      // *** DIAGRAM 3 OF 3 ***

      // Wavefunction(s) for diagram number 3
      FFV1_1<W_ACCESS, CD_ACCESS>( w_fp[3], w_fp[0], COUPs[0], -1.0, cIPD[0], cIPD[1], w_fp[4] );

      // Amplitude(s) for diagram number 3
      FFV1_0<W_ACCESS, A_ACCESS, CD_ACCESS>( w_fp[2], w_fp[4], w_fp[1], COUPs[0], -1.0, &amp_fp[0] );
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
      // Here the code base generated with multichannel support updates numerators_sv and denominators_sv (#473)
#endif
      jamp_sv[0] -= 2. * amp_sv[0];
      ...
      // *** COLOR MATRIX BELOW ***
      // (This method used to be called CPPProcess::matrix_1_gg_gogo()?)

      // The color denominators (initialize all array elements, with ncolor=6)
      // [NB do keep 'static' for these constexpr arrays, see issue #283]
      static constexpr fptype2 denom[ncolor] = { 6, 6, 6, 6, 6, 6 }; // 1-D array[6]
  ...
  CPPProcess::initProc( const std::string& param_card_name )
  {
    ...
    m_masses.push_back( m_pars->ZERO );
    m_masses.push_back( m_pars->mdl_Mgo );
    m_masses.push_back( m_pars->mdl_Mgo );
    // Read physics parameters like masses and couplings from user configuration files (static: initialize once)
    // Then copy them to CUDA constant memory (issue #39) or its C++ emulation in file-scope static memory
    const fptype tIPD[2] = { (fptype)m_pars->mdl_Mgo, (fptype)m_pars->mdl_Wgo };
  ...
  CPPProcess::initProc( const std::string& /*param_card_name*/ )
  {
    ...
    m_masses.push_back( Parameters_MSSM_SLHA2::ZERO );
    m_masses.push_back( Parameters_MSSM_SLHA2::mdl_Mgo );
    m_masses.push_back( Parameters_MSSM_SLHA2::mdl_Mgo );
  }
#endif
