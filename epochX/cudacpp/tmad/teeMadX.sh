...
function usage()
{
  echo "Usage: $0 <processes [-eemumu][-ggtt][-ggttg][-ggttgg][-ggttggg][-guttu][-gqttq]> [-flt|-fltonly|-mix|-mixonly] [-makeonly] [-makeclean] [-rmrdat] [+10x] [-checkonly]" > /dev/stderr
  exit 1
}
...
for arg in $*; do
  if [ "$arg" == "-eemumu" ]; then
    ...
    if [ "$guttu" == "" ]; then procs+=${procs:+ }${arg}; fi
    guttu=$arg
  elif [ "$arg" == "-gqttq" ]; then
