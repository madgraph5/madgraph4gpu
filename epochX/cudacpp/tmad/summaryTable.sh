...
function oneTable()
{
  taglist="FORTRAN CPP/none CPP/sse4 CPP/avx2 CPP/512y CPP/512z CUDA/32"
  parlist="(1) (2-none) (2-sse4) (2-avx2) (2-512y) (2-512z) (3)"
  faclist="1 10 100"
  for proc in $procs; do
    file=tmad/logs_${proc}_${suff}/log_${proc}_${suff}_${fpt}_${inl}_${hrd}.txt
    if [ ! -f $file ]; then continue; fi
    echo "*** FILE $file ***"
    cat $file | awk -vtaglist="$taglist" -vparlist="$parlist" -vfaclist="$faclist" '
      ...
      END{for (itag=1; itag<=ntag; itag++)
          {tag=tags[itag]; printf "| %-8s  |", tag;
