...
#pragma omp parallel for default(none) shared(allmomenta,allMEs,cHel,cIPC,cIPD,ihel,npagV,amp_fp,w_fp) private (amp_sv,w_sv,jamp_sv)
...
#endif
    {
      ...
      ixxxxx<M_ACCESS, W_ACCESS>( momenta, cIPD[0], cHel[ihel][0], +1, w_fp[0], 0 );

      oxxxxx<M_ACCESS, W_ACCESS>( momenta, cIPD[0], cHel[ihel][1], -1, w_fp[1], 1 );

      oxxxxx<M_ACCESS, W_ACCESS>( momenta, cIPD[1], cHel[ihel][2], +1, w_fp[2], 2 );

      ixxxxx<M_ACCESS, W_ACCESS>( momenta, cIPD[1], cHel[ihel][3], -1, w_fp[3], 3 );

      FFV1P0_3<W_ACCESS>( w_fp[0], w_fp[1], cxmake( cIPC[0], cIPC[1] ), 0., 0., w_fp[4] );

      // Amplitude(s) for diagram number 1
      FFV1_0<W_ACCESS, A_ACCESS>( w_fp[3], w_fp[2], w_fp[4], cxmake( cIPC[0], cIPC[1] ), &amp_fp[0] );
      jamp_sv[0] += +1./6.*amp_sv[0];
      jamp_sv[1] -= 1./2.*amp_sv[0];

      // *** COLOR ALGEBRA BELOW ***
      // (This method used to be called CPPProcess::matrix_1_bbx_ttx()?)

      // The color matrix (initialize all array elements, with ncolor=1)
      // [NB do keep 'static' for these constexpr arrays, see issue #283]
      static constexpr fptype denom[ncolor] = {1, 1}; // 1-D array[2]
  ...
  void CPPProcess::initProc( const std::string& param_card_name )
  {
    ...
    const fptype tIPD[2] = { (fptype)m_pars->mdl_MB, (fptype)m_pars->mdl_MT };
#ifdef __CUDACC__
