...
namespace mgOnGpu
{
  ...
  // The 0 entry is a fall back to still write events even if no multi-channel is setup (wrong color selected in that mode) 
    __device__ constexpr int channelId_to_iconfigC[5] = {
  ...
  // This array has N_config <= N_diagrams elements    
    __device__ constexpr bool icolamp[4][3] = {
    {false, false, true}, // iconfigC=0, diag=1
    {true, true, false}, // iconfigC=1, diag=2
    {true, false, false}, // iconfigC=2, diag=3
    {false, true, false}, // iconfigC=3, diag=4
  };
