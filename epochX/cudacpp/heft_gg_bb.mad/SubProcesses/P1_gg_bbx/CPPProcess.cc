...
// MadGraph5_aMC@NLO v. 3.7.1, 2026-04-29
...
#endif
{
  ...
  constexpr int nIPC = 1; // SM independent couplings used in this CPPProcess.cc (FIXME? rename as sm_IndepCoupl?)
  ...
  __device__ const fptype cIPD[nIPD] = { (fptype)Parameters_heft::mdl_MB, (fptype)Parameters_heft::mdl_MH, (fptype)Parameters_heft::mdl_WH };
  __device__ const fptype cIPC[nIPC * 2] = { (fptype)Parameters_heft::GC_74.real(), (fptype)Parameters_heft::GC_74.imag() };
#else
#ifdef MGONGPUCPP_GPUIMPL
...
#ifdef MGONGPU_HARDCODE_PARAM
  __device__ const double* bsmIndepParam = Parameters_heft::mdl_bsmIndepParam;
#else
#ifdef MGONGPUCPP_GPUIMPL
  __device__ __constant__ double bsmIndepParam[Parameters_heft::nBsmIndepParam];
#else
  static double bsmIndepParam[Parameters_heft::nBsmIndepParam];
#endif
#endif
#else
#ifdef MGONGPU_HARDCODE_PARAM
...
#pragma nv_diagnostic pop
  ...
  CPPProcess::initProc( const std::string& param_card_name )
  {
    ...
    const cxtype tIPC[nIPC] = { cxmake( m_pars->GC_74 ) };
#ifdef MGONGPUCPP_GPUIMPL
    gpuMemcpyToSymbol( cIPD, tIPD, nIPD * sizeof( fptype ) );
    gpuMemcpyToSymbol( cIPC, tIPC, nIPC * sizeof( cxtype ) );
#ifdef MGONGPUCPP_NBSMINDEPPARAM_GT_0
    if( Parameters_heft::nBsmIndepParam > 0 )
      gpuMemcpyToSymbol( bsmIndepParam, m_pars->mdl_bsmIndepParam, Parameters_heft::nBsmIndepParam * sizeof( double ) );
#endif
#else
...
#ifdef MGONGPUCPP_NBSMINDEPPARAM_GT_0
    if( Parameters_heft::nBsmIndepParam > 0 )
      memcpy( bsmIndepParam, m_pars->mdl_bsmIndepParam, Parameters_heft::nBsmIndepParam * sizeof( double ) );
#endif
#endif
    ...
    //for ( int i=0; i<Parameters_heft::nBsmIndepParam; i++ ) std::cout << std::setprecision(17) << "m_pars->mdl_bsmIndepParam[i] = " << m_pars->mdl_bsmIndepParam[i] << std::endl;
  }
#else
...
#error "icc is no longer supported: please use icx"
  ...
  select_col( int* allselcol,                    // output: color selection[nevt]
              ...
              const unsigned int* allChannelIds, // input: multichannel channelIds[nevt] (1 to #diagrams); nullptr to disable SDE enhancement (fix #899/#911)
              const int* allIgraph,              // input: per-event MLM graph (0 = no MLM); nullptr if no MLM
              ...
              const int nevt )                   // input: #events (for cuda: nevt == ndim == gpublocks*gputhreads)
  {
    ...
    // SCALAR channelId for the current event (CUDA)
    unsigned int channelId = gpu_channelId( allChannelIds );
    // Per-event MLM graph (0 = no MLM)
    const int igraph = ( allIgraph != nullptr ) ? allIgraph[ievt] : 0;
    // Event-by-event random choice of color #402
    if( channelId != 0 || igraph != 0 ) // no event-by-event choice of color if both channelId and igraph are 0 (fix FPE #783)
    {
      ...
      if( igraph != 0 )
      {
        iconfig = igraph; // use MLM-matched graph directly as iconfig (F-indexed, 1-based)
      }
      else
      {
...
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
    // Event-by-event random choice of color #402
    gpuLaunchKernel( select_col, gpublocks, gputhreads, allselcol, allrndcol, allChannelIds, allIgraph, colAllJamp2s, gpublocks * gputhreads );
#endif
...
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
#define _OMPLIST1 , allDenominators, allNumerators, allChannelIds, allIgraph, mgOnGpu::icolamp, mgOnGpu::channel2iconfig
#else
#define _OMPLIST1
#endif
