...
const int ievt)
{
  ...
  // printf( "%f\n", momenta1d[ipagM*npar*np4*neppM + ipar*np4*neppM +
  // ip4*neppM + ieppM] );
  return momenta1d[ipagM * npar * np4 * neppM + ipar * np4 * neppM + ip4 *
      neppM + ieppM]; // AOSOA[ipagM][ipar][ip4][ieppM]
}
...
const int ipar)  // input: particle# out of npar
{
  ...
  //+++ START EVENT LOOP (where necessary) +++
  {
    ...
    // registers used
    // AV: copying by value (not by ref) seems irrelevant, or slightly slower,
    // in c++
    ...
    if (fmass != 0.)
    {
      const fptype_sv pp = fpmin(pvec0, fpsqrt(pvec1 * pvec1 + pvec2 * pvec2 +
          pvec3 * pvec3));
#ifndef MGONGPU_CPPSIMD
      if (pp == 0.)
      {
    ...
    {
      const fptype_sv sqp0p3 = fpternary((pvec1 == 0. and pvec2 == 0. and pvec3
          < 0.),
      fptype_sv{0}, fpsqrt(fpmax(pvec0 + pvec3, 0.)) * (fptype)nsf); 
      const cxtype_sv chi[2] = 
      {
        cxmake(sqp0p3, 0.), cxternary((sqp0p3 == 0.), 
...
const int ipar)  // input: particle# out of npar
{
  ...
  //+++ START EVENT LOOP (where necessary) +++
  {
    ...
    const fptype_sv pvec3 = pIparIp4Ipag(allmomenta, ipar, 3, ipagV); 
#endif
    // fi[0] = cxmake( -pvec0 * nsf, -pvec2 * nsf ); // AV: BUG! not the same
    // as ixxxxx
    // fi[1] = cxmake( -pvec0 * nsf, -pvec1 * nsf ); // AV: BUG! not the same
    // as ixxxxx
...
const int ipar)  // input: particle# out of npar
{
  ...
  //+++ START EVENT LOOP (where necessary) +++
  {
    ...
    if (vmass != 0.)
    {
      ...
      else
      {
        ...
        if (pt != 0.)
        {
          ...
          vc[3] = cxmake(hel0 * pvec1 * emp - pvec1 * pzpt, -nsvahl * pvec2/pt
              * sqh);
          vc[4] = cxmake(hel0 * pvec2 * emp - pvec2 * pzpt, nsvahl * pvec1/pt *
              sqh);
        }
        else
        {
      ...
      const cxtype_v vcB2_4 = cxmake(0., (fptype)nsvahl * fpternary((pvec3 <
          0), -sqh, sqh)); // AV: removed an abs here
    ...
    else
    {
      ...
      // Branch A: pt != 0.
      const fptype_v pzpt = pvec3/(pp * pt) * sqh * hel; 
      const cxtype_v vcA_3 = cxmake(-pvec1 * pzpt, -(fptype)nsv * pvec2/pt *
          sqh);
      const cxtype_v vcA_4 = cxmake(-pvec2 * pzpt, (fptype)nsv * pvec1/pt *
          sqh);
      // Branch B: pt == 0.
      const cxtype vcB_3 = cxmake(-(fptype)hel * sqh, 0); 
      const cxtype_v vcB_4 = cxmake(0, (fptype)nsv * fpternary((pvec3 < 0),
          -sqh, sqh)); // AV: removed an abs here
      // Choose between the results from branch A and branch B
...
const int ipar)  // input: particle# out of npar
{
  ...
  //+++ START EVENT LOOP (where necessary) +++
  {
    ...
    // AV: copying by value (not by ref) seems faster in cuda, in spite of more
    // registers used
    // AV: copying by value (not by ref) seems irrelevant, or slightly faster,
    // in c++
    const fptype pvec0 = pIparIp4Ievt(allmomenta, ipar, 0, ievt); 
    const fptype pvec1 = pIparIp4Ievt(allmomenta, ipar, 1, ievt); 
    const fptype pvec2 = pIparIp4Ievt(allmomenta, ipar, 2, ievt); 
    ...
    if (fmass != 0.)
    {
      const fptype_sv pp = fpmin(pvec0, fpsqrt((pvec1 * pvec1) + (pvec2 *
          pvec2) + (pvec3 * pvec3)));
#ifndef MGONGPU_CPPSIMD
      if (pp == 0.)
      {
    ...
    {
      const fptype_sv sqp0p3 = fpternary((pvec1 == 0.) and (pvec2 == 0.) and
          (pvec3 < 0.),
...
// 
// __device__
// inline const fptype& pIparIp4Ievt(const fptype * momenta1d,  // input:
// momenta as AOSOA[npagM][npar][4][neppM]
// const int ipar,
// const int ip4,
// const int ievt)
// {
...
// const int ipagM = ievt/neppM;  // #eventpage in this iteration
// const int ieppM = ievt%neppM;  // #event in the current eventpage in this
// iteration
...
// p[0] = sqrt(p[1] * p[1] + p[2] * p[2] + p[3] * p[3] + fmass * fmass); // AV:
// BUG?! (NOT AS IN THE FORTRAN)
...
// {
// vc[3] = cxtype(-hel * sqh, 0.0);
// vc[4] = cxtype(0.0, nsvahl * (p3 < 0) ? - abs(sqh) : abs(sqh)); // BUG issue
// #198
...
// {
// vc[3] = cxtype(-hel * sqh, 0.0);
// vc[4] = cxtype(0.0, nsv * (p3 < 0) ? - abs(sqh) : abs(sqh)); // BUG issue
// #198
