...
function usage()
{
  echo "Usage: $0 <processes [-eemumu][-ggtt][-ggttg][-ggttgg][-ggttggg][-ggttgggg][-gqttq][-heftggbb][-susyggtt][-susyggt1t1][-smeftggtttt]> [-sa] [-noalpaka] [-flt|-fltonly|-mix|-mixonly] [-inl|-inlonly|-inlL|-inlLonly] [-hrd|-hrdonly] [-common|-curhst] [-rmbhst|-bridge] [-makeonly] [-makeclean] [-makej] [-dlp <dyld_library_path>]" # -nofpe is no longer supported
  exit 1
}
...
for arg in $*; do
  if [ "${dlpset}" == "1" ]; then
    ...
    ggttggg=$arg
  elif [ "$arg" == "-ggttgggg" ]; then
    if [ "$ggttgggg" == "" ]; then procs+=${procs:+ }${arg}; fi
    ggttgggg=$arg
  elif [ "$arg" == "-gqttq" ]; then
