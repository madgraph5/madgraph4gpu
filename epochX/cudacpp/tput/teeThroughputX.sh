...
function usage()
{
  echo "Usage: $0 <processes [-eemumu][-ggtt][-ggttg][-ggttgg][-ggttggg][-gqttq][-heftggbb][-susyggtt][-susyggt1t1][-smeftggtttt][-ewdim6udwz]> [-sa] [-noalpaka] [-flt|-fltonly|-mix|-mixonly] [-inl|-inlonly] [-hrd|-hrdonly] [-common|-curhst] [-rmbhst|-bridge] [-makeonly] [-makeclean] [-makej] [-dlp <dyld_library_path>]" # -nofpe is no longer supported
  exit 1
}
...
for arg in $*; do
  if [ "${dlpset}" == "1" ]; then
    ...
    smeftggtttt=$arg
  elif [ "$arg" == "-ewdim6udwz" ]; then
    if [ "$ewdim6udwz" == "" ]; then procs+=${procs:+ }${arg}; fi
    ewdim6udwz=$arg
  elif [ "$arg" == "-sa" ]; then
