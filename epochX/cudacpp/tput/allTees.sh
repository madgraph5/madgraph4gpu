...
# (36/90) Six logs (double/float/mixed x hrd0/hrd1 x inl0) in each of the six SM processes
...
# (48/90) Four extra logs (double/float x hrd0/hrd1 x inl1) only in three of the six SM processes
...
# (60/90) Two extra logs (double/float x hrd0 x inl0 + bridge) in all six SM processes (rebuild from cache)
...
# (66/90) Two extra logs (double/float x hrd0 x inl0 + rmbhst) only in three of the six SM processes (no rebuild needed)
...
# (72/90) Two extra logs (double/float x hrd0 x inl0 + rndhst) only in three of the six SM processes (no rebuild needed)
...
# (78/90) Two extra logs (double/float x hrd0 x inl0 + common) only in three of the six SM processes (no rebuild needed)
...
# (90/90) Four extra logs (double/float x hrd0/hrd1 x inl0) only in the three BSM processes
cmd="./tput/teeThroughputX.sh -flt -hrd -makej -susyggtt -susyggt1t1 -heftggh -makeclean ${opts}"
tmp3=$(mktemp)
if [ "${bsm}" != "-nobsm" ]; then
  $cmd; status=$?
  ls -ltr susy_gg_tt${suff}/lib/build.none_*_inl0_hrd* susy_gg_t1t1${suff}/lib/build.none_*_inl0_hrd* heft_gg_h${suff}/lib/build.none_*_inl0_hrd* | egrep -v '(total|\./|\.build|_common|^$)' > $tmp2
else
  cmd="SKIP '$cmd'"; echo $cmd; status=$?
fi
