...
inline
void ompnumthreadsNotSetMeansOneThread( int debuglevel ) // quiet(-1), info(0), debug(1)
{
 ...
 if( ompnthr == NULL || std::string( ompnthr ).find_first_not_of( "0123456789" ) != std::string::npos || atol( ompnthr ) == 0 )
 {
   if( ompnthr != NULL )
    ...
    if( debuglevel == 1 ) std::cout << "DEBUG: omp_get_num_threads() = " << omp_get_num_threads() << std::endl; // always == 1 here!
    if( debuglevel == 1 ) std::cout << "DEBUG: omp_get_max_threads() = " << omp_get_max_threads() << std::endl;
  }
  else if( debuglevel >= 0 )
    std::cout << "(ompnumthreadsNotSetMeansOneThread) "
