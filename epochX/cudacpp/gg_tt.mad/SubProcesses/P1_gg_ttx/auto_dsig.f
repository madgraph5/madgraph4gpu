...
C     Cannot make a selection with all PDFs to zero, so we return now
      ...
      SUBROUTINE SELECT_GROUPING(IMIRROR,  IPROC, ICONF, WGT, NB_PAGE_MAX)
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     SUMPROB
C     INPUT
C     nb_page_max (number of weight to update)
C     INPUT/OUTPUT
...
C       We are using the grouped_processes grid and it is initialized.
     ...
     $  ALL_CM_RAP, ICONF,IPROC,IMIRROR, ALL_OUT,NB_PAGE_MAX)
C     ******************************************************
C     
C     INPUT: ALL_PP(0:3, NEXTERNAL, NB_PAGE_MAX)
C     INPUT/OUtpUT       ALL_WGT(Nb_Page_Max)
C     nb_page_max = vector size
...
C     and update the couplings accordingly
      CALL UPDATE_SCALE_COUPLING(ALL_P, ALL_WGT, ALL_Q2FACT, NB_PAGE_MAX)
...
C     IF (PASSCUTS(P1)) THEN
     ...
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT)  ! g g > t t~
C     ENDIF
