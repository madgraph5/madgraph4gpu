...
namespace mg5amcGpu
{
    ...
    , m_gputhreads( gputhreads )
  {
    ...
    // Create the "one-helicity" amplitude buffer that will be used for helicity filtering
    m_pHelAmps.reset( new DeviceBufferSimple( mgOnGpu::nx2 * this->nevt() ) );
  ...
  int MatrixElementKernelDevice::computeGoodHelicities()
  {
...
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
    sigmaKin_getGoodHel( m_momenta.data(), m_couplings.data(), m_matrixElements.data(), m_pHelJamps->data(), m_pHelAmps->data(), m_pHelWfs->data(), m_pHelNumerators->data(), m_pHelDenominators->data(), hstIsGoodHel.data(), nevt );
#else
    sigmaKin_getGoodHel( m_momenta.data(), m_couplings.data(), m_matrixElements.data(), m_pHelJamps->data(), m_pHelAmps->data(), m_pHelWfs->data(), hstIsGoodHel.data(), nevt );
#endif
    ...
    // ... Create the "many-helicity" super-buffer of nGoodHel amplitude buffers (dynamically allocated because nGoodHel is determined at runtime)
    // ... (calling reset here deletes the previously created "one-helicity" buffers used for helicity filtering)
    m_pHelAmps.reset( new DeviceBufferSimple( nGoodHel * mgOnGpu::nx2 * nevt ) );
...
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
    const unsigned int* pChannelIds = ( useChannelIds ? m_channelIds.data() : nullptr );
    sigmaKin( m_momenta.data(), m_couplings.data(), m_rndhel.data(), m_rndcol.data(), pChannelIds, m_matrixElements.data(), m_selhel.data(), m_selcol.data(), m_colJamp2s.data(), m_pHelNumerators->data(), m_pHelDenominators->data(), m_pHelMEs->data(), m_pHelJamps->data(), m_pHelAmps->data(), m_pHelWfs->data(), m_helStreams, m_gpublocks, m_gputhreads );
#else
    assert( useChannelIds == false );
    sigmaKin( m_momenta.data(), m_couplings.data(), m_rndhel.data(), m_matrixElements.data(), m_selhel.data(), m_pHelMEs->data(), m_pHelJamps->data(), m_pHelAmps->data(), m_pHelWfs->data(), m_helStreams, m_gpublocks, m_gputhreads );
#endif
#ifdef MGONGPU_CHANNELID_DEBUG
