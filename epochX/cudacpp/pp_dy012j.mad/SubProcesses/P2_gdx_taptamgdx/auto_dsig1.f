...
C     
C     Process: g d~ > ta+ ta- g d~ WEIGHTED<=6 @2
C     Process: g s~ > ta+ ta- g s~ WEIGHTED<=6 @2
C     Process: g b~ > ta+ ta- g b~ WEIGHTED<=6 @2
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $       -PP(3,I))))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g d~ > ta+ ta- g d~
      PD(IPROC)=G1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s~ > ta+ ta- g s~
      PD(IPROC)=G1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g b~ > ta+ ta- g b~
      PD(IPROC)=G1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
