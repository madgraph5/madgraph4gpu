...
// Process: g g > e+ e- d s d~ s~ WEIGHTED<=8 / h
// Process: g g > e+ e- d b d~ b~ WEIGHTED<=8 / h
// Process: g g > e+ e- s b s~ b~ WEIGHTED<=8 / h
// Process: g g > mu+ mu- d s d~ s~ WEIGHTED<=8 / h
// Process: g g > mu+ mu- d b d~ b~ WEIGHTED<=8 / h
// Process: g g > mu+ mu- s b s~ b~ WEIGHTED<=8 / h
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[4], w[9], pars->GC_1, pars->ZERO, pars->ZERO, w[15]); 
  ...
  FFV2_3_2(w[7], w[22], -pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[24]);
  FFV2_3_1(w[4], w[22], -pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[25]);
...
double CPPProcess::matrix_gg_epemdsdxsx_no_h() 
{
  int i, j; 
  // Local variables
