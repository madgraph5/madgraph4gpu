...
// Process: d u~ > ta+ ta- c d u~ c~ WEIGHTED<=8 / h
// Process: d c~ > ta+ ta- u d c~ u~ WEIGHTED<=8 / h
// Process: s u~ > ta+ ta- c s u~ c~ WEIGHTED<=8 / h
// Process: s c~ > ta+ ta- u s c~ u~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dux_taptamcduxcx_no_h(); 
        ...
        t[1] = matrix_dux_taptamcduxcx_no_h(); 
        double tsum = 0; 
...
double CPPProcess::matrix_dux_taptamcduxcx_no_h() 
{
  int i, j; 
  // Local variables
