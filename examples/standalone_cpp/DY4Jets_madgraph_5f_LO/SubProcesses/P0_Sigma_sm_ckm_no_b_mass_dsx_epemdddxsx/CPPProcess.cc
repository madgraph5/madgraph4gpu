...
// Process: d s~ > e+ e- d d d~ s~ WEIGHTED<=8 / h
// Process: d s~ > mu+ mu- d d d~ s~ WEIGHTED<=8 / h
// Process: d b~ > e+ e- d d d~ b~ WEIGHTED<=8 / h
// Process: d b~ > mu+ mu- d d d~ b~ WEIGHTED<=8 / h
// Process: s d~ > e+ e- s s s~ d~ WEIGHTED<=8 / h
// Process: s d~ > mu+ mu- s s s~ d~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dsx_epemdddxsx_no_h(); 
        ...
        t[1] = matrix_dsx_epemdddxsx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1P0_3(w[6], w[5], pars->GC_11, pars->ZERO, pars->ZERO, w[9]); 
  ...
  FFV1P0_3(w[6], w[4], pars->GC_11, pars->ZERO, pars->ZERO, w[36]); 
...
double CPPProcess::matrix_dsx_epemdddxsx_no_h() 
{
  int i, j; 
  // Local variables
