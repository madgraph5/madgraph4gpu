...
// Process: d d~ > e+ e- u u u~ u~ WEIGHTED<=8 / h
// Process: d d~ > e+ e- c c c~ c~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- u u u~ u~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- c c c~ c~ WEIGHTED<=8 / h
// Process: s s~ > e+ e- u u u~ u~ WEIGHTED<=8 / h
// Process: s s~ > e+ e- c c c~ c~ WEIGHTED<=8 / h
// Process: s s~ > mu+ mu- u u u~ u~ WEIGHTED<=8 / h
// Process: s s~ > mu+ mu- c c c~ c~ WEIGHTED<=8 / h
// Process: b b~ > e+ e- u u u~ u~ WEIGHTED<=8 / h
// Process: b b~ > e+ e- c c c~ c~ WEIGHTED<=8 / h
// Process: b b~ > mu+ mu- u u u~ u~ WEIGHTED<=8 / h
// Process: b b~ > mu+ mu- c c c~ c~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ddx_epemuuuxux_no_h(); 
        ...
        t[1] = matrix_ddx_epemuuuxux_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1P0_3(w[6], w[5], pars->GC_11, pars->ZERO, pars->ZERO, w[35]); 
...
double CPPProcess::matrix_ddx_epemuuuxux_no_h() 
{
  int i, j; 
  // Local variables
