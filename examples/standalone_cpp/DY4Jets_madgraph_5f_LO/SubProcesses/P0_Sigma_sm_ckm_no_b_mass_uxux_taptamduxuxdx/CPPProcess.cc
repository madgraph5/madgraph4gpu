...
// Process: u~ u~ > ta+ ta- d u~ u~ d~ WEIGHTED<=8 / h
// Process: u~ u~ > ta+ ta- s u~ u~ s~ WEIGHTED<=8 / h
// Process: u~ u~ > ta+ ta- u~ b u~ b~ WEIGHTED<=8 / h
// Process: c~ c~ > ta+ ta- d c~ c~ d~ WEIGHTED<=8 / h
// Process: c~ c~ > ta+ ta- s c~ c~ s~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxux_taptamduxuxdx_no_h(); 
...
double CPPProcess::matrix_uxux_taptamduxuxdx_no_h() 
{
  int i, j; 
  // Local variables
