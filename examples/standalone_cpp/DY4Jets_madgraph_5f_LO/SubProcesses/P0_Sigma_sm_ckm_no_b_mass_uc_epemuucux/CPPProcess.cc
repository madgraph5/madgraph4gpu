...
// Process: u c > e+ e- u u c u~ WEIGHTED<=8 / h
// Process: u c > mu+ mu- u u c u~ WEIGHTED<=8 / h
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[1], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[13]); 
...
double CPPProcess::matrix_uc_epemuucux_no_h() 
{
  int i, j; 
  // Local variables
