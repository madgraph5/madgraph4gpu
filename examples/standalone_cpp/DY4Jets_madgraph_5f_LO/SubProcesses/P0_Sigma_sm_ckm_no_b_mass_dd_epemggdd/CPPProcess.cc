...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[1], w[9], pars->GC_1, pars->ZERO, pars->ZERO, w[15]); 
  FFV1_1(w[7], w[8], pars->GC_11, pars->ZERO, pars->ZERO, w[16]); 
  FFV1_1(w[6], w[9], pars->GC_1, pars->ZERO, pars->ZERO, w[17]); 
  ...
  FFV2_4_3(w[2], w[3], -pars->GC_51, pars->GC_59, pars->mdl_MZ, pars->mdl_WZ,
      w[26]);
  FFV2_3_1(w[7], w[26], -pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[27]);
...
double CPPProcess::matrix_dd_epemggdd_no_h() 
{
  int i, j; 
  // Local variables
