...
// Process: d d~ > e+ e- s s s~ s~ WEIGHTED<=8 / h
// Process: d d~ > e+ e- b b b~ b~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- s s s~ s~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- b b b~ b~ WEIGHTED<=8 / h
// Process: s s~ > e+ e- d d d~ d~ WEIGHTED<=8 / h
// Process: s s~ > e+ e- b b b~ b~ WEIGHTED<=8 / h
// Process: s s~ > mu+ mu- d d d~ d~ WEIGHTED<=8 / h
// Process: s s~ > mu+ mu- b b b~ b~ WEIGHTED<=8 / h
// Process: b b~ > e+ e- d d d~ d~ WEIGHTED<=8 / h
// Process: b b~ > e+ e- s s s~ s~ WEIGHTED<=8 / h
// Process: b b~ > mu+ mu- d d d~ d~ WEIGHTED<=8 / h
// Process: b b~ > mu+ mu- s s s~ s~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ddx_epemsssxsx_no_h(); 
        ...
        t[1] = matrix_ddx_epemsssxsx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[1], w[10], pars->GC_1, pars->ZERO, pars->ZERO, w[53]); 
...
double CPPProcess::matrix_ddx_epemsssxsx_no_h() 
{
  int i, j; 
  // Local variables
