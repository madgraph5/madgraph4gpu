...
// Process: u c > ta+ ta- u c d d~ WEIGHTED<=8 / h
// Process: u c > ta+ ta- u c s s~ WEIGHTED<=8 / h
// Process: u c > ta+ ta- u c b b~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uc_taptamucddx_no_h(); 
        ...
        t[1] = matrix_uc_taptamucddx_no_h(); 
        double tsum = 0; 
...
double CPPProcess::matrix_uc_taptamucddx_no_h() 
{
  int i, j; 
  // Local variables
