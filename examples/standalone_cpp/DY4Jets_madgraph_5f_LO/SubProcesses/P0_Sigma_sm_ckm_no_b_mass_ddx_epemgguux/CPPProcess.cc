...
// Process: d d~ > e+ e- g g u u~ WEIGHTED<=8 / h
// Process: d d~ > e+ e- g g c c~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- g g u u~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- g g c c~ WEIGHTED<=8 / h
// Process: s s~ > e+ e- g g u u~ WEIGHTED<=8 / h
// Process: s s~ > e+ e- g g c c~ WEIGHTED<=8 / h
// Process: s s~ > mu+ mu- g g u u~ WEIGHTED<=8 / h
// Process: s s~ > mu+ mu- g g c c~ WEIGHTED<=8 / h
// Process: b b~ > e+ e- g g u u~ WEIGHTED<=8 / h
// Process: b b~ > e+ e- g g c c~ WEIGHTED<=8 / h
// Process: b b~ > mu+ mu- g g u u~ WEIGHTED<=8 / h
// Process: b b~ > mu+ mu- g g c c~ WEIGHTED<=8 / h
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[6], w[9], pars->GC_2, pars->ZERO, pars->ZERO, w[15]); 
  ...
  FFV2_5_2(w[7], w[22], pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[26]);
  FFV1P0_3(w[7], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[27]); 
...
double CPPProcess::matrix_ddx_epemgguux_no_h() 
{
  int i, j; 
  // Local variables
  ...
  jamp[1] = +1./2. * (+1./3. * std::complex<double> (0, 1) * amp[0] + 1./3. *
      ...
      * amp[400]);
  jamp[2] = +1./2. * (+std::complex<double> (0, 1) * amp[0] +
  ...
  jamp[3] = +1./2. * (-std::complex<double> (0, 1) * amp[0] -
      ...
      amp[406]);
  jamp[4] = +1./2. * (+1./3. * amp[96] + 1./3. * amp[97] + 1./3. * amp[98] +
  ...
  jamp[5] = +1./2. * (-amp[96] - amp[97] - amp[98] - amp[99] - amp[100] -
  ...
  jamp[7] = +1./2. * (+std::complex<double> (0, 1) * amp[2] +
      ...
      * amp[122] + std::complex<double> (0, 1) * amp[124] - amp[125] - amp[126]
      ...
      amp[407] + amp[406]);
  jamp[8] = +1./2. * (-amp[48] - amp[49] - amp[50] - amp[51] - amp[52] -
  ...
  jamp[9] = +1./2. * (+1./3. * amp[48] + 1./3. * amp[49] + 1./3. * amp[50] +
