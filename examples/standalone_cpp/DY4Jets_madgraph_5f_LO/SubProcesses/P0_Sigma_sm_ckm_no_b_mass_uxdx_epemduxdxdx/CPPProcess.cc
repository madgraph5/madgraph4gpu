...
// Process: u~ d~ > e+ e- d u~ d~ d~ WEIGHTED<=8 / h
// Process: u~ d~ > mu+ mu- d u~ d~ d~ WEIGHTED<=8 / h
// Process: u~ s~ > e+ e- s u~ s~ s~ WEIGHTED<=8 / h
// Process: u~ s~ > mu+ mu- s u~ s~ s~ WEIGHTED<=8 / h
// Process: u~ b~ > e+ e- b u~ b~ b~ WEIGHTED<=8 / h
// Process: u~ b~ > mu+ mu- b u~ b~ b~ WEIGHTED<=8 / h
// Process: c~ d~ > e+ e- d c~ d~ d~ WEIGHTED<=8 / h
// Process: c~ d~ > mu+ mu- d c~ d~ d~ WEIGHTED<=8 / h
// Process: c~ s~ > e+ e- s c~ s~ s~ WEIGHTED<=8 / h
// Process: c~ s~ > mu+ mu- s c~ s~ s~ WEIGHTED<=8 / h
// Process: c~ b~ > e+ e- b c~ b~ b~ WEIGHTED<=8 / h
// Process: c~ b~ > mu+ mu- b c~ b~ b~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxdx_epemduxdxdx_no_h(); 
        ...
        t[1] = matrix_uxdx_epemduxdxdx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[0], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[53]); 
...
double CPPProcess::matrix_uxdx_epemduxdxdx_no_h() 
{
  int i, j; 
  // Local variables
