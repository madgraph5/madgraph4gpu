...
// Process: d~ d~ > ta+ ta- d d~ d~ d~ WEIGHTED<=8 / h
// Process: s~ s~ > ta+ ta- s s~ s~ s~ WEIGHTED<=8 / h
// Process: b~ b~ > ta+ ta- b b~ b~ b~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dxdx_taptamddxdxdx_no_h(); 
...
double CPPProcess::matrix_dxdx_taptamddxdxdx_no_h() 
{
  int i, j; 
  // Local variables
