...
// Process: b b~ > e+ e- d s d~ s~ WEIGHTED<=8 / h
// Process: b b~ > mu+ mu- d s d~ s~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_bbx_epemdsdxsx_no_h(); 
        ...
        t[1] = matrix_bbx_epemdsdxsx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[6], w[10], pars->GC_1, pars->ZERO, pars->ZERO, w[37]); 
...
double CPPProcess::matrix_bbx_epemdsdxsx_no_h() 
{
  int i, j; 
  // Local variables
