...
// Process: u~ u~ > e+ e- d u~ u~ d~ WEIGHTED<=8 / h
// Process: u~ u~ > e+ e- s u~ u~ s~ WEIGHTED<=8 / h
// Process: u~ u~ > e+ e- u~ b u~ b~ WEIGHTED<=8 / h
// Process: u~ u~ > mu+ mu- d u~ u~ d~ WEIGHTED<=8 / h
// Process: u~ u~ > mu+ mu- s u~ u~ s~ WEIGHTED<=8 / h
// Process: u~ u~ > mu+ mu- u~ b u~ b~ WEIGHTED<=8 / h
// Process: c~ c~ > e+ e- d c~ c~ d~ WEIGHTED<=8 / h
// Process: c~ c~ > e+ e- s c~ c~ s~ WEIGHTED<=8 / h
// Process: c~ c~ > e+ e- c~ b c~ b~ WEIGHTED<=8 / h
// Process: c~ c~ > mu+ mu- d c~ c~ d~ WEIGHTED<=8 / h
// Process: c~ c~ > mu+ mu- s c~ c~ s~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxux_epemduxuxdx_no_h(); 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[1], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[27]); 
...
double CPPProcess::matrix_uxux_epemduxuxdx_no_h() 
{
  int i, j; 
  // Local variables
