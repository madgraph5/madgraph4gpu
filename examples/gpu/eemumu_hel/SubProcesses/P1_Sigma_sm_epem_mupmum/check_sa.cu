...
int main(int argc, char **argv) {
  ...
  double* lp = new double[4*3*dim/ncomb];
  ...
  for (int x = 0; x < numiter; ++x) {
    ...
    std::vector<std::vector<double *>> p =
        get_momenta(process.ninitial, energy, process.getMasses(), weight, dim/ncomb);

    // Set momenta for this event
    for (int d = 0; d < dim/ncomb; ++d) {
      for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 3; ++j) {
          lp[i*dim/ncomb*3+j*dim/ncomb+d] = p[d][i][1+j];
        }
      }
    }

    //new
    int num_bytes = 3*4*dim/ncomb * sizeof(double);
    double *allmomenta = 0;
	   ...
	   for (int j =0; j< ncomb; j++){
            if (verbose)
               std::cout << " Matrix element = "
                      		...
                      		<< meHostPtr[i*1 + ncomb* d + j] << " GeV^" << meGeVexponent << std::endl;
          if (perf)
