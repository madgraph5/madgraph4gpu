...
__device__ void ixxxxx(double pvec[3], double fmass, int nhel, int nsf,
  ...
  if (fmass != 0.0) {
    ...
    if (pp == 0.0) {
      ...
      ip = (1 + nh) / 2;
      im = (1 - nh) / 2;
      fi[2] = make_mg5Complex(ip * sqm[ip], 0);
      fi[3] = make_mg5Complex(im * nsf * sqm[ip], 0);
      fi[4] = make_mg5Complex(ip * nsf * sqm[im], 0);
      fi[5] = make_mg5Complex(im * sqm[im], 0);
    } else {
      ...
      pp3 = max(pp + p[3], 0.0);
      chi[0] = make_mg5Complex(sqrt(pp3 * 0.5 / pp), 0);
      if (pp3 == 0.0) {
    ...
    chi[0] = make_mg5Complex(sqp0p3, 0.0);
    if (sqp0p3 == 0.0) {
    ...
    if (nh == 1) {
      ...
      fi[3] = make_mg5Complex(0.0, 0.0);
      fi[4] = chi[0];
      fi[5] = chi[1];
    } else {
      fi[2] = chi[1];
      fi[3] = chi[0];
      fi[4] = make_mg5Complex(0.0, 0.0);
      fi[5] = make_mg5Complex(0.0, 0.0);
    }
  }
...
__device__ void txxxxx(double pvec[3], double tmass, int nhel, int nst,
  ...
  ft[4][0] = make_mg5Complex(p[0] * nst, p[3] * nst);
  ft[5][0] = make_mg5Complex(p[1] * nst, p[2] * nst);

  // construct eps+
  if (nhel >= 0) {
    if (pp == 0) {
      ep[0] = make_mg5Complex(0, 0);
      ep[1] = make_mg5Complex(-sqh, 0);
      ep[2] = make_mg5Complex(0, nst * sqh);
      ep[3] = make_mg5Complex(0, 0);
    } else {
      ep[0] = make_mg5Complex(0, 0);
      ep[3] = make_mg5Complex(pt / pp * sqh, 0);

      if (pt != 0) {
        pzpt = p[3] / (pp * pt) * sqh;
        ep[1] = make_mg5Complex(-p[1] * pzpt, -nst * p[2] / pt * sqh);
        ep[2] = make_mg5Complex(-p[2] * pzpt, nst * p[1] / pt * sqh);
      } else {
        ep[1] = make_mg5Complex(-sqh, 0);
        ep[2] = make_mg5Complex(0, nst * (p[3] < 0) ? -abs(sqh) : abs(sqh));
      }
    }
  }
  ...
  if (nhel <= 0) {
    if (pp == 0) {
      em[0] = make_mg5Complex(0, 0);
      em[1] = make_mg5Complex(sqh, 0);
      em[2] = make_mg5Complex(0, nst * sqh);
      em[3] = make_mg5Complex(0, 0);
    } else {
      em[0] = make_mg5Complex(0, 0);
      em[3] = make_mg5Complex(-pt / pp * sqh, 0);

      if (pt != 0) {
        pzpt = -p[3] / (pp * pt) * sqh;
        em[1] = make_mg5Complex(-p[1] * pzpt, -nst * p[2] / pt * sqh);
        em[2] = make_mg5Complex(-p[2] * pzpt, nst * p[1] / pt * sqh);
      } else {
        em[1] = make_mg5Complex(sqh, 0);
        em[2] = make_mg5Complex(0, nst * (p[3] < 0) ? -abs(sqh) : abs(sqh));
      }
    }
  }
  ...
  if (std::labs(nhel) <= 1) {
    if (pp == 0) {
      e0[0] = make_mg5Complex(0, 0);
      e0[1] = make_mg5Complex(0, 0);
      e0[2] = make_mg5Complex(0, 0);
      e0[3] = make_mg5Complex(1, 0);
    } else {
      emp = p[0] / (tmass * pp);
      e0[0] = make_mg5Complex(pp / tmass, 0);
      e0[3] = make_mg5Complex(p[3] * emp, 0);

      if (pt != 0) {
        e0[1] = make_mg5Complex(p[1] * emp, 0);
        e0[2] = make_mg5Complex(p[2] * emp, 0);
      } else {
        e0[1] = make_mg5Complex(0, 0);
        e0[2] = make_mg5Complex(0, 0);
      }
...
__device__ void vxxxxx(double pvec[3], double vmass, int nhel, int nsv,
  ...
  pp = min(p[0], sqrt(pt2 + (p[3] * p[3])));
  pt = min(pp, sqrt(pt2));
  vc[0] = make_mg5Complex(p[0] * nsv, p[3] * nsv);
  vc[1] = make_mg5Complex(p[1] * nsv, p[2] * nsv);
  if (vmass != 0.0) {
    hel0 = 1.0 - std::abs(hel);
    if (pp == 0.0) {
      vc[2] = make_mg5Complex(0.0, 0.0);
      vc[3] = make_mg5Complex(-hel * sqh, 0.0);
      vc[4] = make_mg5Complex(0.0, nsvahl * sqh);
      vc[5] = make_mg5Complex(hel0, 0.0);
    } else {
      emp = p[0] / (vmass * pp);
      vc[2] = make_mg5Complex(hel0 * pp / vmass, 0.0);
      vc[5] = make_mg5Complex(hel0 * p[3] * emp + hel * pt / pp * sqh, 0.0);
      if (pt != 0.0) {
        pzpt = p[3] / (pp * pt) * sqh * hel;
        vc[3] = make_mg5Complex(hel0 * p[1] * emp - p[1] * pzpt,
                                -nsvahl * p[2] / pt * sqh);
        vc[4] = make_mg5Complex(hel0 * p[2] * emp - p[2] * pzpt,
                                nsvahl * p[1] / pt * sqh);
      } else {
        vc[3] = make_mg5Complex(-hel * sqh, 0.0);
        vc[4] =
            make_mg5Complex(0.0, nsvahl * (p[3] < 0) ? -abs(sqh) : abs(sqh));
      }
    }
  } else {
    pp = p[0];
    ...
    vc[2] = make_mg5Complex(0.0, 0.0);
    vc[5] = make_mg5Complex(hel * pt / pp * sqh, 0.0);
    if (pt != 0.0) {
      pzpt = p[3] / (pp * pt) * sqh * hel;
      vc[3] = make_mg5Complex(-p[1] * pzpt, -nsv * p[2] / pt * sqh);
      vc[4] = make_mg5Complex(-p[2] * pzpt, nsv * p[1] / pt * sqh);
    } else {
      vc[3] = make_mg5Complex(-hel * sqh, 0.0);
      vc[4] = make_mg5Complex(0.0, nsv * (p[3] < 0) ? -abs(sqh) : abs(sqh));
    }
  }
  return;
}
...
__device__ void sxxxxx(double pvec[3], int nss, mg5Complex sc[3]) {
  ...
  sc[1] = make_mg5Complex(p[1] * nss, p[2] * nss);
  return;
}
...
__device__ void oxxxxx(double pvec[3], double fmass, int nhel, int nsf,
  ...
  fo[1] = make_mg5Complex(p[1] * nsf, p[2] * nsf);
  nh = nhel * nsf;
  if (fmass != 0.000) {
    pp = min(p[0], sqrt((p[1] * p[1]) + (p[2] * p[2]) + (p[3] * p[3])));
    if (pp == 0.000) {
      ...
      ip = -((1 - nh) / 2) * nhel;
      im = (1 + nh) / 2 * nhel;
      fo[2] = make_mg5Complex(im * sqm[std::abs(ip)], 0);
      fo[3] = make_mg5Complex(ip * nsf * sqm[std::abs(ip)], 0);
      fo[4] = make_mg5Complex(im * nsf * sqm[std::abs(im)], 0);
      ...
      pp3 = max(pp + p[3], 0.00);
      chi[0] = make_mg5Complex(sqrt(pp3 * 0.5 / pp), 0.00);
      if (pp3 == 0.00) {
        chi[1] = make_mg5Complex(-nh, 0.00);
      } else {
        chi[1] = make_mg5Complex(nh * p[1], -p[2]) / sqrt(2.0 * pp * pp3);
      }
    ...
    chi[0] = make_mg5Complex(sqp0p3, 0.00);
    if (sqp0p3 == 0.000) {
      chi[1] = make_mg5Complex(-nhel, 0.00) * sqrt(2.0 * p[0]);
    } else {
      chi[1] = make_mg5Complex(nh * p[1], -p[2]) / sqp0p3;
    }
    if (nh == 1) {
      fo[2] = chi[0];
      fo[3] = chi[1];
      fo[4] = make_mg5Complex(0.00, 0.00);
      fo[5] = make_mg5Complex(0.00, 0.00);
    } else {
      fo[2] = make_mg5Complex(0.00, 0.00);
      fo[3] = make_mg5Complex(0.00, 0.00);
      fo[4] = chi[1];
      fo[5] = chi[0];
    }
  }
...
__device__ void FFV2_3(mg5Complex F1[], const mg5Complex F2[],
  ...
  V3[5] = denom * (-cI) * (-F1[2] * F2[4] - P3[3] * OM3 * TMP1 + F1[3] * F2[5]);
}
...
__device__ void FFV2_4_0(mg5Complex F1[], const mg5Complex F2[],
  ...
  TMP0 = (F1[2] * (F2[4] * (V3[2] + V3[5]) + F2[5] * (V3[3] + cI * (V3[4]))) +
          F1[3] * (F2[4] * (V3[3] - cI * (V3[4])) + F2[5] * (V3[2] - V3[5])));
  (*vertex) =
      (-1.) * (COUP2 * (+cI * (TMP0) + 2. * cI * (TMP2)) + cI * (TMP0 * COUP1));
}
  ...
  V3[2] = denom * (-2. * cI) *
          (COUP2 * (OM3 * -1. / 2. * P3[0] * (TMP1 + 2. * (TMP3)) +
                    (+1. / 2. * (F1[2] * F2[4] + F1[3] * F2[5]) +
                     F1[4] * F2[2] + F1[5] * F2[3])) +
           1. / 2. *
               (COUP1 * (F1[2] * F2[4] + F1[3] * F2[5] - P3[0] * OM3 * TMP1)));
  V3[3] = denom * (-2. * cI) *
          (COUP2 * (OM3 * -1. / 2. * P3[1] * (TMP1 + 2. * (TMP3)) +
                    (-1. / 2. * (F1[2] * F2[5] + F1[3] * F2[4]) +
                     F1[4] * F2[3] + F1[5] * F2[2])) -
           1. / 2. *
               (COUP1 * (F1[2] * F2[5] + F1[3] * F2[4] + P3[1] * OM3 * TMP1)));
  V3[4] = denom * cI *
  ...
  V3[5] = denom * 2. * cI *
          (COUP2 * (OM3 * 1. / 2. * P3[3] * (TMP1 + 2. * (TMP3)) +
                    (+1. / 2. * (F1[2] * F2[4]) - 1. / 2. * (F1[3] * F2[5]) -
                     F1[4] * F2[2] + F1[5] * F2[3])) +
           1. / 2. *
               (COUP1 * (F1[2] * F2[4] + P3[3] * OM3 * TMP1 - F1[3] * F2[5])));
}
...
__device__ void calculate_wavefunctions(int ihel, double local_mom[4][3],
  ...
  FFV2_4_3(w[1], w[0], make_mg5Complex(cIPC[2], cIPC[3]),
           make_mg5Complex(cIPC[4], cIPC[5]), cIPD[0], cIPD[1], w[4]);
