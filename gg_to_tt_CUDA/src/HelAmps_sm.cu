...
__device__ void ixxxxx(const fptype * allmomenta, const fptype& fmass, const
    int& nhel, const int& nsf, const int& flv,
ALOHAOBJ &fi, 
#ifndef __CUDACC__
const int ievt, 
#endif
const int ipar)  // input: particle# out of npar
{
  ...
  else
  {
    ...
    if (nh == 1)
    {
      fi.W[0] = cxtype(0.0, 0.0); 
      fi.W[1] = cxtype(0.0, 0.0); 
      fi.W[2] = chi[0]; 
      fi.W[3] = chi[1]; 
    }
    else
    {
      ...
      fi.W[2] = cxtype(0.0, 0.0); 
      fi.W[3] = cxtype(0.0, 0.0); 
    }
  }
  //** END LOOP ON IEVT **
...
const int ipar)  // input: particle# out of npar
{
  ...
  if (vmass != 0.0)
  {
    ...
    if (pp == 0.0)
    {
      vc.W[0] = cxtype(0.0, 0.0); 
      vc.W[1] = cxtype(-hel * sqh, 0.0); 
      vc.W[2] = cxtype(0.0, nsvahl * sqh); 
      vc.W[3] = cxtype(hel0, 0.0); 
    }
    else
    {
      emp = p0/(vmass * pp); 
      vc.W[0] = cxtype(hel0 * pp/vmass, 0.0); 
      vc.W[3] = cxtype(hel0 * p3 * emp + hel * pt/pp * sqh, 0.0); 
      if (pt != 0.0)
      {
        pzpt = p3/(pp * pt) * sqh * hel; 
        vc.W[1] = cxtype(hel0 * p1 * emp - p1 * pzpt, 
         - nsvahl * p2/pt * sqh); 
        vc.W[2] = cxtype(hel0 * p2 * emp - p2 * pzpt, 
        nsvahl * p1/pt * sqh); 
      }
      else
      {
        vc.W[1] = cxtype(-hel * sqh, 0.0); 
        vc.W[2] = cxtype(0.0, nsvahl * (p3 < 0) ? - abs(sqh)
        : abs(sqh)); 
      }
    }
  }
  else
  {
    ...
    pt = sqrt((p1 * p1) + (p2 * p2)); 
    vc.W[0] = cxtype(0.0, 0.0); 
    vc.W[3] = cxtype(hel * pt/p0 * sqh, 0.0); 
    if (pt != 0.0)
    {
      pzpt = p3/(p0 * pt) * sqh * hel; 
      vc.W[1] = cxtype(-p1 * pzpt, -nsv * p2/pt * sqh); 
      vc.W[2] = cxtype(-p2 * pzpt, nsv * p1/pt * sqh); 
    }
    else
    {
      vc.W[1] = cxtype(-hel * sqh, 0.0); 
...
__device__ void oxxxxx(const fptype * allmomenta, const fptype& fmass, const
    int& nhel, const int& nsf, int& flv,
ALOHAOBJ &fo, 
#ifndef __CUDACC__
const int ievt, 
#endif
const int ipar)  // input: particle# out of npar
{
  ...
  else
  {
    ...
    if (nh == 1)
    {
      fo.W[0] = chi[0]; 
      fo.W[1] = chi[1]; 
      fo.W[2] = cxtype(0.00, 0.00); 
      fo.W[3] = cxtype(0.00, 0.00); 
    }
    else
    {
      fo.W[0] = cxtype(0.00, 0.00); 
      fo.W[1] = cxtype(0.00, 0.00); 
      fo.W[2] = chi[1]; 
      fo.W[3] = chi[0]; 
    }
  }
