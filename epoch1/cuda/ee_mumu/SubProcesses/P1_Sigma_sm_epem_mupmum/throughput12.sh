...
function usage()
{
  echo "Usage: $0 [-nocpp|[-omp|-noomp][-avxall][-nocuda]] [-nohet] [-ep2] [-3a3b] [-ggttgg] [-div] [-req] [-flt|-fltonly] [-inl|-inlonly] [-detailed] [-v]"
  exit 1
}
...
#=====================================
if [ "${het}" == "1" ]; then 
  exes="$exes ../../../../../epoch1/cuda/ee_mumu/SubProcesses/P1_Sigma_sm_epem_mupmum/build.512y_${fptype}_inl${helinl}/hcheck.exe"
fi
