...
if [ "${redrel##*release 7}" != "${redrel}" ]; then
  source /cvmfs/sft.cern.ch/lcg/releases/clang/17.0.1-be287/x86_64-centos7/setup.sh
elif [ "${redrel##*release 9}" != "${redrel}" ]; then
  source /cvmfs/sft.cern.ch/lcg/releases/clang/17.0.1-be287/x86_64-el9/setup.sh
else
  echo "ERROR! RedHat release ${redrel} is not supported by ${BASH_SOURCE}"
  return 1
fi
###export ALLOW_UNSUPPORTED_COMPILER_IN_CUDA=1
export CC=$scrdir/mg-lcg-clang-17.0.1
export CXX=$scrdir/mg-lcg-clang++-17.0.1
