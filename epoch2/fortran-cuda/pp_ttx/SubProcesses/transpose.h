...
std::unique_ptr<T[], CudaHstDeleter<T>> hstMakeUnique(int n_bytes) {
 ...
 template<typename T>
__global__ void dev_transpose(const T* inpArr, T* outArr,
...
void Matrix<T>::fill(T* arr) {
  ...
  T (*aos)[m_part][m_mome] =
    (T (*)[m_part][m_mome]) arr; // was -> m_hstInpArray.get();
  ...
  for (int i = 0; i < m_evnt * m_part * m_mome; ++i) {
    if (i && i % m_mome == 0) std::cout << std::endl;
    if (i && i % (m_mome * m_part) == 0) std::cout << std::endl;
    std::cout << aos_p[i] << " ";
  }
  std::cout << std::endl;
#endif // DEBUG
 ...
 */
template<typename T>
void Matrix<T>::hst_transpose(T* arr) {

  std::unique_ptr<T[], CudaHstDeleter<T>> hstInpArray = hstMakeUnique<T>(m_arrbytes);
  std::unique_ptr<T[], CudaDevDeleter<T>> devInpArray = devMakeUnique<T>(m_arrbytes);
  std::unique_ptr<T[], CudaHstDeleter<T>> hstOutArray = hstMakeUnique<T>(m_arrbytes);
  std::unique_ptr<T[], CudaDevDeleter<T>> devOutArray = devMakeUnique<T>(m_arrbytes);

  checkCuda( cudaMemcpy( devInpArray.get(), hstInpArray.get(),
                         m_arrbytes, cudaMemcpyHostToDevice));

  dev_transpose<<<gpublocks, gputhreads>>>(devInpArray.get(),
                                           devOutArray.get(),
                                           m_evnt, m_part,
                                           m_mome, m_strd);

  checkCuda( cudaMemcpy( hstOutArray.get(), devOutArray.get(),
                         m_arrbytes, cudaMemcpyDeviceToHost));

  #ifdef DEBUG
  std::cout << std::string(80, '*') << std::endl;
  T *aosoa_p = (T*)hstOutArray.get();
  for (int i = 0; i < evnt_n*part_n*mome_n; ++i) {
    if (i && i % m_strd == 0) std::cout << ", ";
    if (i && i % (m_mome * m_strd) == 0) std::cout << std::endl;
    if (i && i % (m_part * m_mome * m_strd) == 0) std::cout << std::endl;
    std::cout << aosoa_p[i] << " ";
  }
