...
template <typename T = fptype> struct CudaDevDeleter {
...
template <typename T = fptype> struct CudaHstDeleter {
...
template <typename T = fptype>
inline std::unique_ptr<T[]> hstMakeUnique(std::size_t N) {
  return std::unique_ptr<T[]>{new T[N]()};
};

#ifdef MGONGPU_CPPSIMD

template <> inline std::unique_ptr<fptype_v[]> hstMakeUnique(std::size_t N) {
  return std::unique_ptr<fptype_v[]>{new fptype_v[N / neppV]()};
};
