...
public:
  ...
  void dump(std::ostream &ostr = std::cout, bool json = false) {
    ...
    for (auto ip : m_partitionTimers) {
      total += ip.second;
      // if ( ipart != 0 && ipart+1 != m_partitionTimers.size() ) totalBut2 +=
      // ip.second;
      if (ip.first[0] == '1' || ip.first[0] == '2' || ip.first[0] == '3')
        total123 += ip.second;
      if (ip.first[0] == '2' || ip.first[0] == '3')
        total23 += ip.second;
      if (ip.first[0] == '1')
        total1 += ip.second;
      if (ip.first[0] == '2')
    ...
    if (json) {
      ...
      ostr << std::setprecision(
          6);             // set precision (default=6): affects all floats
      ...
      ostr << std::fixed; // fixed format: affects all floats
      for (auto ip : m_partitionTimers)
        ostr << std::setw(maxsize) << ip.first << " : " << std::setw(12)
             << ip.second << " sec" << std::endl;
      ostr << std::setw(maxsize) << totalKey << " : " << std::setw(12) << total
           << " sec" << std::endl
           << std::setw(maxsize) << total123Key << " : " << std::setw(12)
           << total123 << " sec" << std::endl
           << std::setw(maxsize) << total23Key << " : " << std::setw(12)
           << total23 << " sec" << std::endl
           << std::setw(maxsize) << total1Key << " : " << std::setw(12)
           << total1 << " sec" << std::endl
           << std::setw(maxsize) << total2Key << " : " << std::setw(12)
           << total2 << " sec" << std::endl
           << std::setw(maxsize) << total3Key << " : " << std::setw(12)
           << total3 << " sec" << std::endl
           << std::setw(maxsize) << total3aKey << " : " << std::setw(12)
           << total3a << " sec" << std::endl;
      ostr << std::defaultfloat; // default format: affects all floats
    }
  }
