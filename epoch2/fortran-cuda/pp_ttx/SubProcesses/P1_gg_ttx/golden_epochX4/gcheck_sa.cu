...
void debug_me_is_abnormal( const fptype& me, int ievtALL )
{
  std::cout << "DEBUG[" << ievtALL << "]"
            << " ME=" << me << " fpisabnormal=" << fp_is_abnormal(me)
            << " fpclass=" << fp_show_class(me) << " (me==me)=" << (me == me)
            << " (me==me+1)=" << (me == me + 1) << " isnan=" << std::isnan(me)
            << " isfinite=" << std::isfinite(me)
            << " isnormal=" << std::isnormal(me) << " is0=" << (me == 0)
            << " is1=" << (me == 1) << " abs(ME)=" << std::abs(me)
            << " isnan=" << std::isnan(std::abs(me)) << std::endl;
}

int usage(char *argv0, int ret = 1) {
  std::cout << "Usage: " << argv0
            << " [--verbose|-v] [--debug|-d] [--performance|-p] [--json|-j]"
            << " [#gpuBlocksPerGrid #gpuThreadsPerBlock] #iterations"
            << std::endl
            << std::endl;
  std::cout << "The number of events per iteration is #gpuBlocksPerGrid * "
               "#gpuThreadsPerBlock"
            << std::endl;
  std::cout << "(also in CPU/C++ code, where only the product of these two "
               "parameters counts)"
            << std::endl
            << std::endl;
  std::cout << "Summary stats are always computed: '-p' and '-j' only control "
               "their printout"
            << std::endl;
  std::cout
      << "The '-d' flag only enables NaN/abnormal warnings and OMP debugging"
      << std::endl;
#ifndef __CUDACC__
#ifdef _OPENMP
  std::cout << std::endl
            << "Use the OMP_NUM_THREADS environment variable to control OMP "
               "multi-threading"
            << std::endl;
  std::cout
      << "(OMP multithreading will be disabled if OMP_NUM_THREADS is not set)"
      << std::endl;
#endif
#endif
  return ret;
...
int main(int argc, char **argv) {
  ...
  for (int argn = 1; argn < argc; ++argn) {
    if (strcmp(argv[argn], "--verbose") == 0 || strcmp(argv[argn], "-v") == 0)
      ...
      perf = true;
    else if (strcmp(argv[argn], "--json") == 0 || strcmp(argv[argn], "-j") == 0)
      json = true;
  ...
  if (gputhreads % neppR != 0) {
    std::cout << "ERROR! #threads/block should be a multiple of neppR=" << neppR
              << std::endl;
    return usage(argv[0]);
  }
  ...
  if (gputhreads % neppM != 0) {
    std::cout << "ERROR! #threads/block should be a multiple of neppM=" << neppM
              << std::endl;
    return usage(argv[0]);
  }
  ...
  if (debug) {
    std::cout << "DEBUG: omp_get_num_threads() = " << omp_get_num_threads()
              << std::endl; // always == 1 here!
    std::cout << "DEBUG: omp_get_max_threads() = " << omp_get_max_threads()
              << std::endl;
    std::cout << "DEBUG: ${OMP_NUM_THREADS}    = '"
              << (ompnthr == 0 ? "[not set]" : ompnthr) << "'" << std::endl;
  }
  if (ompnthr == NULL ||
      std::string(ompnthr).find_first_not_of("0123456789") !=
          std::string::npos ||
      atol(ompnthr) == 0) {
    if (ompnthr != NULL)
      std::cout << "WARNING! OMP_NUM_THREADS is invalid: will use only 1 thread"
                << std::endl;
    else if (debug)
      std::cout << "DEBUG: OMP_NUM_THREADS is not set: will use only 1 thread"
                << std::endl;
  ...
  // Fail gently and avoid "Illegal instruction (core dumped)" if the host does
  // not support the requested AVX [NB: this prevents a crash on pmpe04 but not
  // on some github CI nodes]
  auto supportsAvx = []() {
...
#endif
    if (tag == "none")
      std::cout << "INFO: The application does not require the host to support "
                   "any AVX feature"
                << std::endl;
    else if (ok)
  ...
  const int nevt =
      ndim; // number of events in one iteration == number of GPU threads
  const int nevtALL =
      niter * nevt; // total number of ALL events in all iterations
  ...
  struct CudaTearDown {
    ...
    ~CudaTearDown() {
      ...
      checkCuda(cudaDeviceReset()); // this is needed by cuda-memcheck
                                    // --leak-check full
    }
    bool _print{false};
  } cudaTearDown(debug);
  ...
  const fptype energy =
      1500; // historical default, Ecms = 1500 GeV = 1.5 TeV (above the Z peak)
  // const fptype energy = 91.2; // Ecms = 91.2 GeV (Z peak)
  // const fptype energy = 0.100; // Ecms = 100 MeV (well below the Z peak, pure
  // em scattering)
  ...
  // Memory structures for random numbers, momenta, matrix elements and weights
  // on host and device
  ...
  const int nRnarray =
      np4 * nparf *
      nevt; // (NB: AOSOA layout with nevt=npagR*neppR events per iteration)
  const int nMomenta =
      np4 * npar * nevt; // (NB: nevt=npagM*neppM for AOSOA layouts)
  const int nWeights = nevt;
  const int nMEs = nevt; // FIXME: assume process.nprocesses == 1 (eventually:
                         // nMEs = nevt * nprocesses?)

#if defined MGONGPU_CURAND_ONHOST or defined MGONGPU_COMMONRAND_ONHOST or      \
    not defined __CUDACC__
  auto hstRnarray = hstMakeUnique<fptype>(
      nRnarray); // AOSOA[npagR][nparf][np4][neppR] (NB: nevt=npagR*neppR)
#endif
  auto hstMomenta = hstMakeUnique<fptype_sv>(
      nMomenta); // AOSOA[npagM][npar][np4][neppM] (NB: nevt=npagM*neppM)
  auto hstIsGoodHel = hstMakeUnique<bool>(ncomb);
  auto hstWeights = hstMakeUnique<fptype>(nWeights);
  auto hstMEs = hstMakeUnique<fptype_sv>(
      nMEs); // AOSOA[npagM][neppM] (NB: nevt=npagM*neppM)

#ifdef __CUDACC__
  auto devRnarray = devMakeUnique<fptype>(
      nRnarray); // AOSOA[npagR][nparf][np4][neppR] (NB: nevt=npagR*neppR)
  auto devMomenta = devMakeUnique<fptype>(
      nMomenta); // AOSOA[npagM][npar][np4][neppM] (NB: nevt=npagM*neppM)
  auto devIsGoodHel = devMakeUnique<bool>(ncomb);
  auto devWeights = devMakeUnique<fptype>(nWeights);
  auto devMEs =
      devMakeUnique<fptype>(nMEs); // AOSOA[npagM][neppM] (NB: nevt=npagM*neppM)
  ...
  std::unique_ptr<fptype[]> matrixelementALL(
      new fptype[nevtALL]); // FIXME: assume process.nprocesses == 1
  ...
  CommonRandomNumbers::startGenerateAsync(commonRandomPromises, nRnarray,
                                          niter);
#else
  ...
  for (int iiter = 0; iiter < niter; ++iiter) {
    ...
    // [NB This should not be necessary using the host API: "Generation
    // functions can be called multiple times on the same generator to generate
    // successive blocks of results. For pseudorandom generators, multiple calls
    // to generation functions will yield the same result as a single call with
    // a large size."]
    ...
    // --- 1b. Generate all relevant numbers to build nevt events (i.e. nevt
    // phase space points) on the host
    const std::string rngnKey = "1b GenRnGen";
    timermap.start(rngnKey);
#ifdef MGONGPU_COMMONRAND_ONHOST
    std::vector<fptype> commonRnd =
        commonRandomPromises[iiter].get_future().get();
    assert(nRnarray == static_cast<int>(commonRnd.size()));
    // NB (PR #45): memcpy is strictly needed only in CUDA (copy to pinned
    // memory), but keep it also in C++ for consistency
    ...
    // NB (PR #45): this cudaMemcpy would involve an intermediate memcpy to
    // pinned memory, if hstRnarray was not already cudaMalloc'ed
    checkCuda(cudaMemcpy(devRnarray.get(), hstRnarray.get(), nbytesRnarray,
...
#ifdef __CUDACC__
    grambo2toNm0::getMomentaInitial<<<gpublocks, gputhreads>>>(
        energy, devMomenta.get());
#else
    rambo2toNm0::getMomentaInitial(energy, hstMomenta.get(), nevt);
#endif
    // std::cout << "Got initial momenta" << std::endl;

    // --- 2b. Fill in momenta of final state particles using the RAMBO
    // algorithm on the device (i.e. map random numbers to final-state particle
    // momenta for each of nevt events)
    const std::string rfinKey = "2b RamboFin";
    rambtime += timermap.start(rfinKey);
#ifdef __CUDACC__
    grambo2toNm0::getMomentaFinal<<<gpublocks, gputhreads>>>(
        energy, devRnarray.get(), devMomenta.get(), devWeights.get());
#else
    rambo2toNm0::getMomentaFinal(energy, hstRnarray.get(), hstMomenta.get(),
                                 hstWeights.get(), nevt);
#endif
    ...
    checkCuda(cudaMemcpy(hstWeights.get(), devWeights.get(), nbytesWeights,
                         cudaMemcpyDeviceToHost));

    // --- 2d. CopyDToH Momenta
    const std::string cmomKey = "2d CpDTHmom";
    rambtime += timermap.start(cmomKey);
    checkCuda(cudaMemcpy(hstMomenta.get(), devMomenta.get(), nbytesMomenta,
                         cudaMemcpyDeviceToHost));
#endif
    ...
    // 0d. (Only on the first iteration) Get good helicities [renamed as 0d:
    // this is initialisation!] 3a. Evaluate MEs on the device 3b. Copy MEs back
    // from device to host
...
#else
    gProc::sigmaKin<<<gpublocks, gputhreads, ntpbMAX * sizeof(float)>>>(
        devMomenta.get(), devMEs.get());
#endif
    checkCuda(cudaPeekAtLastError());
    ...
    checkCuda(cudaMemcpy(hstMEs.get(), devMEs.get(), nbytesMEs,
                         cudaMemcpyDeviceToHost));
    ...
    if (verbose) {
      ...
      if (perf)
        std::cout << "Wave function time: " << wavetime << std::endl;
    }

    for (int ievt = 0; ievt < nevt;
         ++ievt) // Loop over all events in this iteration
    {
      if (verbose) {
        // Display momenta
        const int ipagM = ievt / neppM; // #eventpage in this iteration
        const int ieppM =
            ievt % neppM; // #event in the current eventpage in this iteration
        ...
        for (int ipar = 0; ipar < npar; ipar++) {
          ...
          std::cout
              ...
              << std::setw(14)
              << hstMomenta[ipagM * npar * np4 * neppM + ipar * np4 * neppM +
                            0 * neppM + ieppM] // AOSOA[ipagM][ipar][0][ieppM]
              << std::setw(14)
              << hstMomenta[ipagM * npar * np4 * neppM + ipar * np4 * neppM +
                            1 * neppM + ieppM] // AOSOA[ipagM][ipar][1][ieppM]
              << std::setw(14)
              << hstMomenta[ipagM * npar * np4 * neppM + ipar * np4 * neppM +
                            2 * neppM + ieppM] // AOSOA[ipagM][ipar][2][ieppM]
              << std::setw(14)
              << hstMomenta[ipagM * npar * np4 * neppM + ipar * np4 * neppM +
                            3 * neppM + ieppM] // AOSOA[ipagM][ipar][3][ieppM]
#else
              << std::setw(14)
              << hstMomenta[ipagM * npar * np4 + ipar * np4 + 0]
                           [ieppM] // AOSOA[ipagM][ipar][0][ieppM]
              << std::setw(14)
              << hstMomenta[ipagM * npar * np4 + ipar * np4 + 1]
                           [ieppM] // AOSOA[ipagM][ipar][1][ieppM]
              << std::setw(14)
              << hstMomenta[ipagM * npar * np4 + ipar * np4 + 2]
                           [ieppM] // AOSOA[ipagM][ipar][2][ieppM]
              << std::setw(14)
              << hstMomenta[ipagM * npar * np4 + ipar * np4 + 3]
                           [ieppM] // AOSOA[ipagM][ipar][3][ieppM]
#endif
              << std::endl
...
#endif
                  << " GeV^" << meGeVexponent
                  << std::endl; // FIXME: assume process.nprocesses == 1
        std::cout << std::string(SEP79, '-') << std::endl;
      }
      // Fill the arrays with ALL MEs and weights
#ifndef MGONGPU_CPPSIMD
      matrixelementALL[iiter * nevt + ievt] =
          hstMEs[ievt]; // FIXME: assume process.nprocesses == 1
#else
      matrixelementALL[iiter * nevt + ievt] =
          hstMEs[ievt / neppM]
                [ievt % neppM]; // FIXME: assume process.nprocesses == 1
#endif
  ...
  for (int ievtALL = 0; ievtALL < nevtALL; ++ievtALL) {
    ...
    // - check.exe/commonrand: ME[310744,451171,3007871,3163868,4471038,5473927]
    // with fast math
    // - check.exe/curand: ME[578162,1725762,2163579,5407629,5435532,6014690]
    // with fast math
    // - gcheck.exe/curand: ME[596016,1446938] with fast math
    // Debug NaN/abnormal issues
    // if ( ievtALL == 310744 ) // this ME is abnormal both with and without
    // fast math
    //  debug_me_is_abnormal( matrixelementALL[ievtALL], ievtALL );
    // if ( ievtALL == 5473927 ) // this ME is abnormal only with fast math
    //  debug_me_is_abnormal( matrixelementALL[ievtALL], ievtALL );
    // Compute min/max
    if (fp_is_zero(matrixelementALL[ievtALL]))
      nzero++;
    if (fp_is_abnormal(matrixelementALL[ievtALL])) {
      if (debug) // only printed out with "-p -d" (matrixelementALL is not
                 // filled without -p)
        std::cout << "WARNING! ME[" << ievtALL << "] is NaN/abnormal"
                  << std::endl;
      nabn++;
      continue;
    }
  ...
  if (perf) {
    ...
    std::string nprocall;
    std::unique_ptr<FILE, decltype(&pclose)> nprocpipe(
        popen("nproc --all", "r"), pclose);
    if (!nprocpipe)
      throw std::runtime_error("`nproc --all` failed?");
    std::array<char, 128> nprocbuf;
    while (fgets(nprocbuf.data(), nprocbuf.size(), nprocpipe.get()) != nullptr)
      nprocall += nprocbuf.data();
#endif
    ...
    std::cout
        ...
        << "FP precision                = DOUBLE (NaN/abnormal=" << nabn
        << ", zero=" << nzero << ")" << std::endl
#elif defined MGONGPU_FPTYPE_FLOAT
        << "FP precision                = FLOAT (NaN/abnormal=" << nabn
        << ", zero=" << nzero << ")" << std::endl
#endif
#ifdef __CUDACC__
        ...
        << (neppR == 1 ? " == AOS" : "") << " [HARDCODED FOR REPRODUCIBILITY]"
        << std::endl
...
#elif defined __AVX2__
        << "Internal loops fptype_sv    = VECTOR[" << neppV
        << "] ('avx2': AVX2, 256bit)" << cxtref
        << std::endl
#elif defined __SSE4_2__
        << "Internal loops fptype_sv    = VECTOR[" << neppV
#ifdef __PPC__
        << "] ('sse4': PPC VSX, 128bit)" << cxtref
        << std::endl
#else
        << "] ('sse4': SSE4.2, 128bit)" << cxtref
        << std::endl
#endif
#else
#error Internal error: unknown SIMD build configuration
...
#endif
#ifdef _OPENMP
        << "OMP threads / `nproc --all` = " << omp_get_max_threads() << " / "
        << nprocall // includes a newline
#endif
#endif
        //<< "MatrixElements compiler     = " << process.getCompiler() <<
        // std::endl
        << std::string(SEP79, '-') << std::endl
        ...
        << " )  sec" << std::endl
        << "MeanTimeInMatrixElems       = ( " << meanwtim
        << std::string(16, ' ') << " )  sec" << std::endl
        << "[Min,Max]TimeInMatrixElems  = [ " << minwtim << " ,  " << maxwtim
        << " ]  sec"
        << std::endl
        //<< "StdDevTimeInMatrixElems     = ( " << stdwtim << std::string(16, '
        //') << " )  sec" << std::endl
        << "TotalTime[MECalcOnly]  (3a) = ( " << sumw3atim
        << std::string(16, ' ') << " )  sec" << std::endl
        << "MeanTimeInMECalcOnly        = ( " << meanw3atim
        << std::string(16, ' ') << " )  sec" << std::endl
        << "[Min,Max]TimeInMECalcOnly   = [ " << minw3atim << " ,  "
        << maxw3atim << " ]  sec"
        << std::endl
        //<< "StdDevTimeInMECalcOnly      = ( " << stdw3atim << std::string(16,
        //' ') << " )  sec" << std::endl
        << std::string(SEP79, '-')
        << std::endl
    ...
    std::cout << std::string(SEP79, '*') << std::endl
              ...
              << stdelem / sqrt(nevtALL - nabn) << " )  GeV^" << meGeVexponent
  ...
  if (json) {
    std::string jsonFileName = std::to_string(jsondate) + "-perf-test-run" +
                               std::to_string(jsonrun) + ".json";
    ...
    jsonFile
        ...
        << std::to_string(sumgtim + sumrtim + sumwtim) << " sec\"," << std::endl
        << "\"TotalTime[Rambo+ME] (23)\": \""
        << std::to_string(sumrtim + sumwtim) << " sec\"," << std::endl
        << "\"TotalTime[RndNumGen] (1)\": \"" << std::to_string(sumgtim)
        << " sec\"," << std::endl
        << "\"TotalTime[Rambo] (2)\": \"" << std::to_string(sumrtim)
        << " sec\"," << std::endl
        << "\"TotalTime[MatrixElems] (3)\": \"" << std::to_string(sumwtim)
        << " sec\"," << std::endl
        << "\"MeanTimeInMatrixElems\": \"" << std::to_string(meanwtim)
        << " sec\"," << std::endl
        << "\"MinTimeInMatrixElems\": \"" << std::to_string(minwtim)
        << " sec\"," << std::endl
        << "\"MaxTimeInMatrixElems\": \"" << std::to_string(maxwtim)
        << " sec\","
        << std::endl
        //<< "ProcessID:                = " << getpid() << std::endl
        //<< "NProcesses                = " << process.nprocesses << std::endl
        ...
        << "\"NumMatrixElems(notAbnormal)\": " << nevtALL - nabn << ","
        << std::endl
    ...
    timermap.dump(
        jsonFile,
        true); // NB For the active json timer this dumps a partial total
