...
C
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION D1(NB_PAGE),U1(NB_PAGE),S1(NB_PAGE),C1(NB_PAGE)
      DOUBLE PRECISION CX2(NB_PAGE),SX2(NB_PAGE),UX2(NB_PAGE)
     $ ,DX2(NB_PAGE)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     Continue only if IMODE is 0, 4 or 5
      ...
      DO IVEC=1,NB_PAGE
        IF (ABS(LPP(IB(1))).GE.1) THEN
          LP=SIGN(1,LPP(IB(1)))
          D1(IVEC)=PDG2PDF(ABS(LPP(IB(1))),1*LP, 1,ALL_XBK(IB(1),IVEC)
     $     ,DSQRT(ALL_Q2FACT(1, IVEC)))
          U1(IVEC)=PDG2PDF(ABS(LPP(IB(1))),2*LP, 1,ALL_XBK(IB(1),IVEC)
     $     ,DSQRT(ALL_Q2FACT(1, IVEC)))
          S1(IVEC)=PDG2PDF(ABS(LPP(IB(1))),3*LP, 1,ALL_XBK(IB(1),IVEC)
     $     ,DSQRT(ALL_Q2FACT(1, IVEC)))
          C1(IVEC)=PDG2PDF(ABS(LPP(IB(1))),4*LP, 1,ALL_XBK(IB(1),IVEC)
     $     ,DSQRT(ALL_Q2FACT(1, IVEC)))
        ENDIF
        IF (ABS(LPP(IB(2))).GE.1) THEN
          LP=SIGN(1,LPP(IB(2)))
          CX2(IVEC)=PDG2PDF(ABS(LPP(IB(2))),-4*LP, 1,ALL_XBK(IB(2)
     $     ,IVEC),DSQRT(ALL_Q2FACT(2, IVEC)))
          SX2(IVEC)=PDG2PDF(ABS(LPP(IB(2))),-3*LP, 1,ALL_XBK(IB(2)
     $     ,IVEC),DSQRT(ALL_Q2FACT(2, IVEC)))
          UX2(IVEC)=PDG2PDF(ABS(LPP(IB(2))),-2*LP, 1,ALL_XBK(IB(2)
     $     ,IVEC),DSQRT(ALL_Q2FACT(2, IVEC)))
          DX2(IVEC)=PDG2PDF(ABS(LPP(IB(2))),-1*LP, 1,ALL_XBK(IB(2)
     $     ,IVEC),DSQRT(ALL_Q2FACT(2, IVEC)))
        ENDIF
      ENDDO
      ALL_PD(0,:) = 0D0
