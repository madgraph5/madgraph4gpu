...
const int array_bytes = evnt_n * part_n * mome_n * sizeof(arr_t);
...
struct CudaDevDeleter {
  void operator()(arr_t* mem) {
    checkCuda( cudaFree( mem ) );
  }
};
std::unique_ptr<arr_t[], CudaDevDeleter> devMakeUnique(int n_bytes) {
  arr_t* tmp = nullptr;
  checkCuda( cudaMalloc( &tmp, n_bytes ) );
  return std::unique_ptr<arr_t[], CudaDevDeleter>{ tmp };
}
...
std::unique_ptr<arr_t[], CudaHstDeleter> hstMakeUnique(int n_bytes) {
  arr_t* tmp = nullptr;
  checkCuda( cudaMallocHost( &tmp, n_bytes ) );
  return std::unique_ptr<arr_t[], CudaHstDeleter>{ tmp };
};

#endif // __CUDACC__


__global__ void dev_transpose(const arr_t* inpArr, arr_t* outArr,
...
public:
  ...
  void fill() {
    ...
    arr_t (*aos)[part_n][mome_n] = (arr_t (*)[part_n][mome_n]) m_hstInpArray.get();
    ...
    arr_t *aos_p = (arr_t*)m_hstInpArray.get();
...
#ifdef DEBUG
   std::cout << std::string(80, '*') << std::endl;
   arr_t *aosoa_p = (arr_t*)m_hstOutArray.get();
   for (int i = 0; i < evnt_n*part_n*mome_n; ++i) {
...
private:
  std::unique_ptr<arr_t[], CudaHstDeleter> m_hstInpArray;
  std::unique_ptr<arr_t[], CudaDevDeleter> m_devInpArray;
  std::unique_ptr<arr_t[], CudaHstDeleter> m_hstOutArray;
  std::unique_ptr<arr_t[], CudaDevDeleter> m_devOutArray;
};
