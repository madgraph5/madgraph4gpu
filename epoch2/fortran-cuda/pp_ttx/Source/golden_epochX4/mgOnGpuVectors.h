...
#else
typedef fptype fptype_v
    __attribute__((vector_size(neppV * sizeof(fptype)))); // RRRR
#endif

#ifdef __clang__
// If set: return a pair of (fptype&, fptype&) by non-const reference in
// cxtype_v::operator[] This is forbidden in clang ("non-const reference cannot
// bind to vector element") See also
// https://stackoverflow.com/questions/26554829
//#define MGONGPU_HAS_CXTYPE_REF 1 // clang test (compilation fails also on
//clang 12.0, issue #182)
...
// NB: the alternative "clang" implementation is simpler: it simply does not
// have class cxtype_ref
class cxtype_ref {
public:
  cxtype_ref() = delete;
...
#ifdef MGONGPU_HAS_CXTYPE_REF
  // NB: the alternative "clang" implementation is simpler: it simply does not
  // have any operator[] NB: ** do NOT implement operator[] to return a value:
  // it does not fail the build (why?) and gives unexpected results! **
  cxtype_ref operator[](size_t i) const {
    return cxtype_ref(m_real[i], m_imag[i]);
  }
#endif
...
#else // gcc
#if defined MGONGPU_FPTYPE_DOUBLE
typedef long int bool_v
    __attribute__((vector_size(neppV * sizeof(long int)))); // bbbb
#elif defined MGONGPU_FPTYPE_FLOAT
...
inline std::ostream &operator<<(std::ostream &out, const cxtype_v &v) {
  ...
  for (int i = 1; i < neppV; i++)
    std::cout << ", " << cxmake(v.real()[i], v.imag()[i]);
#endif
  out << " }";
  return out;
}
...
inline cxtype_v cxmake(const fptype_v &r, const fptype_v &i) {
  return cxtype_v{r, i};
}
...
inline const cxtype_v cxconj(const cxtype_v &c) {
...
inline cxtype_v operator+(const fptype_v &a, const cxtype_v &b) {
  return cxmake(a + b.real(), b.imag());
}

inline cxtype_v operator+(const cxtype_v &a, const fptype_v &b) {
...
inline cxtype_v operator-(const cxtype_v &a, const fptype &b) {
  return cxmake(a.real() - b, a.imag());
}
...
inline cxtype_v operator*(const cxtype &a, const fptype_v &b) {
  return cxmake(a.real() * b, a.imag() * b);
}
...
inline fptype_v fpternary(const bool_v &mask, const fptype_v &a,
                          const fptype_v &b) {
  fptype_v out;
  for (int i = 0; i < neppV; i++)
...
inline fptype_v fpternary(const bool_v &mask, const fptype &a,
                          const fptype_v &b) {
  fptype_v out;
  for (int i = 0; i < neppV; i++)
    out[i] = (mask[i] ? a : b[i]);
...
inline cxtype_v cxternary(const bool_v &mask, const cxtype_v &a,
                          const cxtype_v &b) {
#ifdef MGONGPU_HAS_CXTYPE_REF
  cxtype_v out;
  for (int i = 0; i < neppV; i++)
...
inline cxtype_v cxternary(const bool_v &mask, const cxtype &a,
                          const cxtype_v &b) {
#ifdef MGONGPU_HAS_CXTYPE_REF
  cxtype_v out;
  for (int i = 0; i < neppV; i++)
...
inline fptype_v fpmax(const fptype_v &a, const fptype_v &b) {
  return fpternary((b < a), a, b);
}

inline fptype_v fpmax(const fptype_v &a, const fptype &b) {
  return fpternary((b < a), a, b);
}
...
inline fptype_v fpmin(const fptype_v &a, const fptype_v &b) {
...
inline bool maskor(const bool_v &mask) {
  bool out = false;
  for (int i = 0; i < neppV; i++)
    out = out || mask[i];
  return out;
...
inline bool maskor(const bool &mask) { return mask; }
...
inline __device__ void print(const cxtype &c) {
  printf("[%f, %f]\n", cxreal(c), cximag(c));
}
...
inline __host__ __device__ fptype fpternary(const bool &mask, const fptype &a,
                                            const fptype &b) {
  return (mask ? a : b);
}

inline __host__ __device__ cxtype cxternary(const bool &mask, const cxtype &a,
                                            const cxtype &b) {
  return (mask ? a : b);
}
