...
double SLHABlock::get_entry( std::vector<int> indices, double def_val )
{
  ...
  {
    std::cout << "Warning: No such entry in " << _name << ", using default value " << def_val << std::endl;
    return def_val;
  }
...
void SLHAReader::read_slha_file( std::string file_name, bool verbose )
{
  ...
  else
  {
    ...
    else
    {
      ...
      if ( param_card.good() ) { std::cout << "Opened slha file " << file_name2 << " for reading" << std::endl; }
      else
      {
  ...
  while ( param_card.good() )
  {
    ...
    if ( line != "" && line[0] != '#' )
    {
      if ( block != "" )
      {
        ...
        if ( linestr2 >> dindex1 >> dindex2 >> value && dindex1 == int( dindex1 ) and dindex2 == int( dindex2 ) )
        {
