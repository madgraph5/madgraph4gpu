  ...
  if (param_card.good()) {
    ...
    if (!getenv(envpath)) {
      ...
      std::cout << "WARNING! Card file '" << file_name << "' does not exist:"
                << " look for the file in directory $" << envpath << "='"
                << getenv(envpath) << "'" << std::endl;
      const std::string file_name2 =
          std::filesystem::path(getenv(envpath)) /
          std::filesystem::path(file_name).filename();
      param_card.open(file_name2.c_str(), std::ifstream::in);
      if (param_card.good()) {
        std::cout << "Opened slha file " << file_name2 << " for reading"
                  << std::endl;
      } else {
        std::cout << "ERROR! Card file '" << file_name2 << "' does not exist"
                  << std::endl;
        throw "Error while opening param card";
      }
    }
    ...
    if (line != "" && line[0] != '#') {
      ...
      if (line.find("decay ") == 0) {
        ...
        if (linestr >> pdg_code >> value)
          ...
          std::cout << "Warning: Wrong format for decay block " << line
                    << std::endl;
        continue;
      }
    }
