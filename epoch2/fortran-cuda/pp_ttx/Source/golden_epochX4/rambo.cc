...
// Simplified rambo version for 2 to N (with N>=2) processes with massless
// particles
#ifdef __CUDACC__
...
// [NB: the output buffer includes both initial and final momenta, but only
// initial momenta are filled in]
__global__ void getMomentaInitial(
...
// [NB: the output buffer includes both initial and final momenta, but only
// initial momenta are filled in]
__global__ void getMomentaFinal(
    const fptype energy,      // input: energy
  ...
  fptype(*rnarray)[nparf][np4][neppR] = (fptype(*)[nparf][np4][neppR])
      rnarray1d;                    // cast to multiD array pointer (AOSOA)
  const int neppM = mgOnGpu::neppM; // AOSOA layout: constant at compile-time
  fptype(*momenta)[npar][np4][neppM] = (fptype(*)[npar][np4][neppM])
      momenta1d; // cast to multiD array pointer (AOSOA)
...
#endif
  {
    ...
    const int ieppR =
        ievt % neppR; // #event in the current eventpage in this iteration
    const int ipagM = ievt / neppM; // #eventpage in this iteration
    const int ieppM =
        ievt % neppM; // #event in the current eventpage in this iteration
    ...
    const fptype rmas =
        sqrt(pow(r[0], 2) - pow(r[3], 2) - pow(r[2], 2) - pow(r[1], 2));
    ...
    for (int iparf = 0; iparf < nparf; iparf++) {
      ...
      for (int i4 = 1; i4 < np4; i4++)
        momenta[ipagM][iparf + npari][i4][ieppM] =
...
void createGenerator(curandGenerator_t *pgen) {
  // [NB Timings are for GenRnGen host|device (cpp|cuda) generation of 256*32*1
  // events with nproc=1: rn(0) is host=0.0012s]
  const curandRngType_t type =
      CURAND_RNG_PSEUDO_MTGP32; // 0.00082s | 0.00064s (FOR FAST TESTS)
  // const curandRngType_t type = CURAND_RNG_PSEUDO_XORWOW;        // 0.049s   |
  // 0.0016s const curandRngType_t type = CURAND_RNG_PSEUDO_MRG32K3A;      //
  // 0.71s    | 0.0012s  (better but slower, especially in c++) const
  // curandRngType_t type = CURAND_RNG_PSEUDO_MT19937;       // 21s      | 0.021s
  // const curandRngType_t type = CURAND_RNG_PSEUDO_PHILOX4_32_10; // 0.024s   |
  // 0.00026s (used to segfault?)
...
#endif
  // checkCurand( curandSetGeneratorOrdering( *pgen,
  // CURAND_ORDERING_PSEUDO_LEGACY ) ); // CUDA 11
...
// Bulk-generate (using curand) the random numbers needed to process nevt events
// in rambo
// ** NB: the random numbers are always produced in the same order and are
// interpreted as an AOSOA AOSOA: rnarray[npagR][nparf][np4][neppR] where
// nevt=npagR*neppR
void generateRnarray(curandGenerator_t gen, // input: curand generator
                     fptype rnarray1d[],    // input: random numbers in [0,1] as
                                            // AOSOA[npagR][nparf][4][neppR]
                     const int nevt)        // input: #events
{
  ...
  // rnarray1d[i*4+1], rnarray1d[i*4+2], rnarray1d[i*4+3] );
}
