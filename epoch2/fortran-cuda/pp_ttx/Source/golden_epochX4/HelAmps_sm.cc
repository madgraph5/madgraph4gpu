...
__device__ inline const fptype &pIparIp4Ievt(
    const fptype *momenta, // input: momenta as AOSOA[npagM][npar][4][neppM]
    const int ipar, const int ip4, const int ievt) {
  using mgOnGpu::np4;
  using mgOnGpu::npar;
  ...
  // printf( "%f\n", momenta[ipagM*npar*np4*neppM + ipar*np4*neppM + ip4*neppM +
  // ieppM] );
  return momenta[ipagM * npar * np4 * neppM + ipar * np4 * neppM + ip4 * neppM +
                 ieppM]; // AOSOA[ipagM][ipar][ip4][ieppM]
  // fptype (*momenta)[npar][np4][neppM] = (fptype (*)[npar][np4][neppM])
  // momenta; // cast to multiD array pointer (AOSOA) return
  // momenta[ipagM][ipar][ip4][ieppM]; // this seems ~1-2% faster in eemumu C++?
}
// TEMPORARY during epochX step3! Eventually add "#endif" (or better "#else")
// here

#ifndef __CUDACC__
// Return by value: it seems a tiny bit faster than returning a reference (both
// for scalar and vector), not clear why
inline fptype_sv pIparIp4Ipag(
    const fptype_sv *momenta, // input: momenta as AOSOA[npagM][npar][4][neppM]
    const int ipar, const int ip4, const int ipagM) {
  ...
  // NB: this is the same as "pIparIp4Ievt( momenta, ipar, ip4, ipagM )" for
  // neppM==1 with neppV==1
  using mgOnGpu::np4;
  using mgOnGpu::npar;
  // printf( "%f\n", momenta[ipagM*npar*np4 + ipar*np4 + ip4] );
  return momenta[ipagM * npar * np4 + ipar * np4 +
                 ip4]; // AOSOA[ipagM][ipar][ip4][ieppM]
#endif
}
#endif
...
__device__ void
ixxxxx(const fptype_sv *momenta,
       ...
       const int ipar) // input: particle# out of npar
{
  ...
  // +++ START EVENT LOOP (where necessary) +++
  {
    ...
    // AV: copying by value (not by ref) seems faster in cuda, in spite of more
    // registers used AV: copying by value (not by ref) seems irrelevant, or
    // slightly slower, in c++
    ...
    // const fptype pvec0 = fpsqrt( pvec1 * pvec1 + pvec2 * pvec2 + pvec3 *
    // pvec3 ); // AV: BUG?! (NOT AS IN THE FORTRAN)
    const fptype pvec0 = pIparIp4Ievt(
        momenta, ipar, 0, ievt); // AV: BUG FIX (DO AS IN THE FORTRAN)
#else
    ...
    if (fmass != 0.) {
      const fptype_sv pp =
          fpmin(pvec0, fpsqrt(pvec1 * pvec1 + pvec2 * pvec2 + pvec3 * pvec3));
#ifndef MGONGPU_CPPSIMD
      if (pp == 0.) {
        // NB: Do not use "abs" for floats! It returns an integer with no build
        // warning! Use std::abs!
        fptype sqm[2] = {fpsqrt(std::abs(fmass)),
                         0.}; // possibility of negative fermion masses
        // sqm[1] = ( fmass < 0. ? -abs( sqm[0] ) : abs( sqm[0] ) ); // AV: why
        // abs here?
        sqm[1] = (fmass < 0. ? -sqm[0] : sqm[0]); // AV: removed an abs here
        const int ip =
            (1 + nh) / 2; // NB: Fortran sqm(0:1) also has indexes 0,1 as in C++
        const int im =
            (1 - nh) / 2; // NB: Fortran sqm(0:1) also has indexes 0,1 as in C++
        ...
        const int ip = (1 + nh) / 2; // NB: Fortran is (3+nh)/2 because omega(2)
                                     // has indexes 1,2 and not 0,1
        const int im = (1 - nh) / 2; // NB: Fortran is (3-nh)/2 because omega(2)
                                     // has indexes 1,2 and not 0,1
        const fptype sfomega[2] = {sf[0] * omega[ip], sf[1] * omega[im]};
        const fptype pp3 = fpmax(pp + pvec3, 0.);
        const cxtype chi[2] = {
            cxmake(fpsqrt(pp3 * (fptype)0.5 / pp), 0.),
            (pp3 == 0. ? cxmake(-nh, 0.)
      ...
      // warning! Use std::abs!
      fptype sqm[2] = {
          fpsqrt(std::abs(fmass)),
          0}; // possibility of negative fermion masses (NB: SCALAR!)
      sqm[1] =
          (fmass < 0 ? -sqm[0] : sqm[0]); // AV: removed an abs here (as above)
      const cxtype fiA_2 = ip * sqm[ip]; // scalar cxtype: real part initialised
                                         // from fptype, imag part = 0
      const cxtype fiA_3 =
          im * nsf * sqm[ip]; // scalar cxtype: real part initialised from
                              // fptype, imag part = 0
      const cxtype fiA_4 =
          ip * nsf * sqm[im]; // scalar cxtype: real part initialised from
                              // fptype, imag part = 0
      const cxtype fiA_5 = im * sqm[im]; // scalar cxtype: real part initialised
                                         // from fptype, imag part = 0
      // Branch B: pp != 0.
      const fptype sf[2] = {fptype(1 + nsf + (1 - nsf) * nh) * (fptype)0.5,
                            fptype(1 + nsf - (1 - nsf) * nh) * (fptype)0.5};
      ...
      const cxtype_v chi[2] = {
          cxmake(fpsqrt(pp3 * 0.5 / pp), 0),
          cxternary((pp3 == 0.), cxmake(-nh, 0),
      ...
      const fptype_sv sqp0p3 =
          fpternary((pvec1 == 0. and pvec2 == 0. and pvec3 < 0.), fptype_sv{0},
                    fpsqrt(fpmax(pvec0 + pvec3, 0.)) * (fptype)nsf);
      const cxtype_sv chi[2] = {
          cxmake(sqp0p3, 0.),
          cxternary((sqp0p3 == 0.),
...
__device__ void
ipzxxx(const fptype_sv *momenta,
  ...
  // +++ START EVENT LOOP (where necessary) +++
  {
    ...
    // AV: copy by value (not by ref) as this seems faster in cuda for other
    // functions
    const fptype pvec3 = pIparIp4Ievt(momenta, ipar, 3, ievt);
#else
...
// ASSUMPTIONS: (FMASS == 0) and (PX == PY == 0 and E == -PZ > 0)
__device__ void
imzxxx(const fptype_sv *momenta,
       // const fptype fmass,           // ASSUME fermion mass==0
       const int nhel, // input: -1 or +1 (helicity of fermion)
       const int nsf,  // input: +1 (particle) or -1 (antiparticle)
       cxtype_sv fi[],
  ...
  // +++ START EVENT LOOP (where necessary) +++
  {
    ...
    // AV: copying by value (not by ref) seems to give the same performance in
    // both cuda and c++
    const fptype pvec3 = pIparIp4Ievt(momenta, ipar, 3, ievt);
#else
...
__device__ void
ixzxxx(const fptype_sv *momenta,
       // const fptype fmass,           // ASSUME fermion mass==0
       const int nhel, // input: -1 or +1 (helicity of fermion)
       const int nsf,  // input: +1 (particle) or -1 (antiparticle)
       cxtype_sv fi[],
  ...
  // +++ START EVENT LOOP (where necessary) +++
  {
    ...
    const fptype_sv pvec3 = pIparIp4Ipag(momenta, ipar, 3, ipagV);
#endif
    // fi[0] = cxmake( -pvec0 * nsf, -pvec2 * nsf ); // AV: BUG! not the same as
    // ixxxxx fi[1] = cxmake( -pvec0 * nsf, -pvec1 * nsf ); // AV: BUG! not the
    // same as ixxxxx
    fi[0] = cxmake(-pvec0 * (fptype)nsf, -pvec3 * (fptype)nsf); // AV: BUG FIX
    fi[1] = cxmake(-pvec1 * (fptype)nsf, -pvec2 * (fptype)nsf); // AV: BUG FIX
    const int nh = nhel * nsf;
    // const float sqp0p3 = sqrtf( pvec0 + pvec3 ) * nsf; // AV: why force a
    // float here?
...
__device__ void vxxxxx(const fptype_sv *momenta,
                       const fptype vmass, // input: vector boson mass
                       const int nhel, // input: -1, 0 (only if vmass!=0) or +1
                                       // (helicity of vector boson)
    ...
    if (vmass != 0.) {
      ...
      if (pp == 0.) {
        ...
        if (pt != 0.) {
          ...
          vc[3] = cxmake(hel0 * pvec1 * emp - pvec1 * pzpt,
                         -nsvahl * pvec2 / pt * sqh);
          vc[4] = cxmake(hel0 * pvec2 * emp - pvec2 * pzpt,
                         nsvahl * pvec1 / pt * sqh);
        } else {
          vc[3] = cxmake(-hel * sqh, 0.);
          // NB: Do not use "abs" for floats! It returns an integer with no
          // build warning! Use std::abs!
          // vc[4] = cxmake( 0., nsvahl * ( pvec3 < 0. ? -std::abs( sqh ) :
          // std::abs( sqh ) ) ); // AV: why abs here?
          vc[4] = cxmake(
              0.,
              nsvahl * (pvec3 < 0. ? -sqh : sqh)); // AV: removed an abs here
        }
      }
#else
      // Branch A: pp == 0.
      ...
      const cxtype_v vcB_5 =
          cxmake(hel0 * pvec3 * emp + hel * pt / pp * sqh, 0);
      // Branch B1: pp != 0. and pt != 0.
      const fptype_v pzpt = pvec3 / (pp * pt) * sqh * hel;
      const cxtype_v vcB1_3 = cxmake(hel0 * pvec1 * emp - pvec1 * pzpt,
                                     -(fptype)nsvahl * pvec2 / pt * sqh);
      const cxtype_v vcB1_4 = cxmake(hel0 * pvec2 * emp - pvec2 * pzpt,
                                     (fptype)nsvahl * pvec1 / pt * sqh);
      // Branch B2: pp != 0. and pt == 0.
      const cxtype vcB2_3 = cxmake(-hel * sqh, 0.);
      const cxtype_v vcB2_4 = cxmake(
          0., (fptype)nsvahl *
                  fpternary((pvec3 < 0), -sqh, sqh)); // AV: removed an abs here
      // Choose between the results from branch A and branch B (and from branch
      // B1 and branch B2)
      const bool_v mask = (pp == 0.);
      ...
      const fptype_sv &pp = pvec0; // NB: rewrite the following as in Fortran,
                                   // using pp instead of pvec0
      ...
      if (pt != 0.) {
        ...
        // NB: Do not use "abs" for floats! It returns an integer with no build
        // warning! Use std::abs!
        // vc[4] = cxmake( 0, nsv * ( pvec3 < 0. ? -std::abs( sqh ) : std::abs(
        // sqh ) ) ); // AV why abs here?
        vc[4] = cxmake(
            0., nsv * (pvec3 < 0. ? -sqh : sqh)); // AV: removed an abs here
      }
#else
      // Branch A: pt != 0.
      const fptype_v pzpt = pvec3 / (pp * pt) * sqh * hel;
      const cxtype_v vcA_3 =
          cxmake(-pvec1 * pzpt, -(fptype)nsv * pvec2 / pt * sqh);
      const cxtype_v vcA_4 =
          cxmake(-pvec2 * pzpt, (fptype)nsv * pvec1 / pt * sqh);
      // Branch B: pt == 0.
      const cxtype vcB_3 = cxmake(-(fptype)hel * sqh, 0);
      const cxtype_v vcB_4 =
          cxmake(0, (fptype)nsv * fpternary((pvec3 < 0), -sqh,
                                            sqh)); // AV: removed an abs here
      // Choose between the results from branch A and branch B
...
__device__ void sxxxxx(const fptype_sv *momenta,
                       const fptype, // WARNING: input "smass" unused (missing
                                     // in Fortran) - scalar boson mass
                       const int,    // WARNING: input "nhel" unused (missing in
                                     // Fortran) - scalar has no helicity!
...
__device__ void
oxxxxx(const fptype_sv *momenta,
       const fptype fmass, // input: fermion mass
       const int nhel,     // input: -1 or +1 (helicity of fermion)
       const int nsf,      // input: +1 (particle) or -1 (antiparticle)
       cxtype_sv fo[],
  ...
  // +++ START EVENT LOOP (where necessary) +++
  {
    ...
    const int nh = nhel * nsf;
    if (fmass != 0.) {
      const fptype_sv pp = fpmin(
          pvec0, fpsqrt((pvec1 * pvec1) + (pvec2 * pvec2) + (pvec3 * pvec3)));
#ifndef MGONGPU_CPPSIMD
      if (pp == 0.) {
        // NB: Do not use "abs" for floats! It returns an integer with no build
        // warning! Use std::abs!
        ...
        const int ip =
            -((1 - nh) / 2) *
            nhel; // NB: Fortran sqm(0:1) also has indexes 0,1 as in C++
        const int im =
            (1 + nh) / 2 *
            nhel; // NB: Fortran sqm(0:1) also has indexes 0,1 as in C++
      ...
      const fptype_sv sqp0p3 =
          fpternary((pvec1 == 0.) and (pvec2 == 0.) and (pvec3 < 0.), 0,
                    fpsqrt(fpmax(pvec0 + pvec3, 0.)) * (fptype)nsf);
      const cxtype_sv chi[2] = {
          cxmake(sqp0p3, 0.),
          cxternary((sqp0p3 == 0.), cxmake(-nhel, 0.) * fpsqrt(2. * pvec0),
                    cxmake((fptype)nh * pvec1, -pvec2) / sqp0p3)};
      if (nh == 1) {
        fo[2] = chi[0];
        fo[3] = chi[1];
...
// ASSUMPTIONS: (FMASS == 0) and (PX == PY == 0 and E == +PZ > 0)
__device__ void
opzxxx(const fptype_sv *momenta,
       // const fptype fmass,           // ASSUME fermion mass==0
       const int nhel, // input: -1 or +1 (helicity of fermion)
       const int nsf,  // input: +1 (particle) or -1 (antiparticle)
       cxtype_sv fo[],
...
// ASSUMPTIONS: (FMASS == 0) and (PX == PY == 0 and E == -PZ > 0)
__device__ void
omzxxx(const fptype_sv *momenta,
       // const fptype fmass,           // ASSUME fermion mass==0
       const int nhel, // input: -1 or +1 (helicity of fermion)
       const int nsf,  // input: +1 (particle) or -1 (antiparticle)
       cxtype_sv fo[],
  ...
  // +++ START EVENT LOOP (where necessary) +++
  {
    ...
    const fptype_sv pvec3 = pIparIp4Ipag(momenta, ipar, 3, ipagV);
#endif
    fo[0] = cxmake(-pvec3 * (fptype)nsf,
                   pvec3 * (fptype)nsf); // remember pvec0 == -pvec3
    fo[1] = cxzero_sv();
    const int nh = nhel * nsf;
...
// ASSUMPTIONS: (FMASS == 0) and (PT > 0)
__device__ void
oxzxxx(const fptype_sv *momenta,
       // const fptype fmass,           // ASSUME fermion mass==0
       const int nhel, // input: -1 or +1 (helicity of fermion)
       const int nsf,  // input: +1 (particle) or -1 (antiparticle)
       cxtype_sv fo[],
...
// FIXME: move these inside each FFV function? Cuda tput[MECalcOnly] seems 3%
// slower for eemumu?
...
// Compute the output wavefunction 'V1[6]' from the input wavefunctions V2[6],
// V3[6]
__device__ void VVV1P0_1(const cxtype_sv V2[], const cxtype_sv V3[],
                         const cxtype COUP, const fptype M1, const fptype W1,
                         cxtype_sv V1[]) {
  mgDebug(0, __FUNCTION__);
  const cxtype cI = cxmake(0., 1.);
  const fptype_sv P2[4] = {+cxreal(V2[0]), +cxreal(V2[1]), +cximag(V2[1]),
                           +cximag(V2[0])};
  const fptype_sv P3[4] = {+cxreal(V3[0]), +cxreal(V3[1]), +cximag(V3[1]),
                           +cximag(V3[0])};
  V1[0] = +V2[0] + V3[0];
  V1[1] = +V2[1] + V3[1];
  const fptype_sv P1[4] = {-cxreal(V1[0]), -cxreal(V1[1]), -cximag(V1[1]),
                           -cximag(V1[0])};
  const cxtype_sv TMP0 =
      (V3[2] * P1[0] - V3[3] * P1[1] - V3[4] * P1[2] - V3[5] * P1[3]);
  const cxtype_sv TMP1 =
      (V3[2] * P2[0] - V3[3] * P2[1] - V3[4] * P2[2] - V3[5] * P2[3]);
  const cxtype_sv TMP2 =
      (P1[0] * V2[2] - P1[1] * V2[3] - P1[2] * V2[4] - P1[3] * V2[5]);
  const cxtype_sv TMP3 =
      (V2[2] * P3[0] - V2[3] * P3[1] - V2[4] * P3[2] - V2[5] * P3[3]);
  const cxtype_sv TMP4 =
      (V3[2] * V2[2] - V3[3] * V2[3] - V3[4] * V2[4] - V3[5] * V2[5]);
  const cxtype_sv denom =
      COUP / ((P1[0] * P1[0]) - (P1[1] * P1[1]) - (P1[2] * P1[2]) -
              (P1[3] * P1[3]) - M1 * (M1 - cI * W1));
  V1[2] = denom * (TMP4 * (-cI * (P2[0]) + cI * (P3[0])) +
                   (V2[2] * (-cI * (TMP0) + cI * (TMP1)) +
                    V3[2] * (+cI * (TMP2)-cI * (TMP3))));
  V1[3] = denom * (TMP4 * (-cI * (P2[1]) + cI * (P3[1])) +
                   (V2[3] * (-cI * (TMP0) + cI * (TMP1)) +
                    V3[3] * (+cI * (TMP2)-cI * (TMP3))));
  V1[4] = denom * (TMP4 * (-cI * (P2[2]) + cI * (P3[2])) +
                   (V2[4] * (-cI * (TMP0) + cI * (TMP1)) +
                    V3[4] * (+cI * (TMP2)-cI * (TMP3))));
  V1[5] = denom * (TMP4 * (-cI * (P2[3]) + cI * (P3[3])) +
                   (V2[5] * (-cI * (TMP0) + cI * (TMP1)) +
                    V3[5] * (+cI * (TMP2)-cI * (TMP3))));
  mgDebug(1, __FUNCTION__);
  return;
}
...
// Compute the output amplitude 'vertex' from the input wavefunctions F1[6],
// F2[6], V3[6]
__device__ void FFV1_0(const cxtype_sv F1[], const cxtype_sv F2[],
                       const cxtype_sv V3[], const cxtype COUP,
                       cxtype_sv *vertex) {
  mgDebug(0, __FUNCTION__);
  const cxtype cI = cxmake(0., 1.);
  const cxtype_sv TMP5 =
...
// Compute the output wavefunction 'F1[6]' from the input wavefunctions F2[6],
// V3[6]
__device__ void FFV1_1(const cxtype_sv F2[], const cxtype_sv V3[],
                       const cxtype COUP, const fptype M1, const fptype W1,
                       cxtype_sv F1[]) {
  mgDebug(0, __FUNCTION__);
  const cxtype cI = cxmake(0., 1.);
  F1[0] = +F2[0] + V3[0];
  F1[1] = +F2[1] + V3[1];
  const fptype_sv P1[4] = {-cxreal(F1[0]), -cxreal(F1[1]), -cximag(F1[1]),
                           -cximag(F1[0])};
  const cxtype_sv denom =
      COUP / ((P1[0] * P1[0]) - (P1[1] * P1[1]) - (P1[2] * P1[2]) -
              (P1[3] * P1[3]) - M1 * (M1 - cI * W1));
  F1[2] =
      denom * cI *
      (F2[2] * (P1[0] * (-V3[2] + V3[5]) + (P1[1] * (V3[3] - cI * (V3[4])) +
                                            (P1[2] * (+cI * (V3[3]) + V3[4]) +
                                             P1[3] * (-V3[2] + V3[5])))) +
       (F2[3] * (P1[0] * (V3[3] + cI * (V3[4])) +
                 (P1[1] * (-one) * (V3[2] + V3[5]) +
                  (P1[2] * (-one) * (+cI * (V3[2] + V3[5])) +
                   P1[3] * (V3[3] + cI * (V3[4]))))) +
  ...
  F1[5] =
      ...
      (F2[4] * (P1[0] * (-V3[3] + cI * (V3[4])) +
                (P1[1] * (V3[2] + V3[5]) +
                 (P1[2] * (-one) * (+cI * (V3[2] + V3[5])) +
                  P1[3] * (-V3[3] + cI * (V3[4]))))) +
       (F2[5] * (P1[0] * (-V3[2] + V3[5]) + (P1[1] * (V3[3] + cI * (V3[4])) +
                                             (P1[2] * (-cI * (V3[3]) + V3[4]) +
                                              P1[3] * (-V3[2] + V3[5])))) +
...
// Compute the output wavefunction 'F2[6]' from the input wavefunctions F1[6],
// V3[6]
__device__ void FFV1_2(const cxtype_sv F1[], const cxtype_sv V3[],
                       const cxtype COUP, const fptype M2, const fptype W2,
                       cxtype_sv F2[]) {
  mgDebug(0, __FUNCTION__);
  const cxtype cI = cxmake(0., 1.);
  F2[0] = +F1[0] + V3[0];
  F2[1] = +F1[1] + V3[1];
  const fptype_sv P2[4] = {-cxreal(F2[0]), -cxreal(F2[1]), -cximag(F2[1]),
                           -cximag(F2[0])};
  const cxtype_sv denom =
      COUP / ((P2[0] * P2[0]) - (P2[1] * P2[1]) - (P2[2] * P2[2]) -
              (P2[3] * P2[3]) - M2 * (M2 - cI * W2));
  F2[2] =
      denom * cI *
      (F1[2] * (P2[0] * (V3[2] + V3[5]) +
                (P2[1] * (-one) * (V3[3] + cI * (V3[4])) +
  ...
  F2[3] =
      denom * (-cI) *
      (F1[2] * (P2[0] * (-one) * (V3[3] + cI * (V3[4])) +
                (P2[1] * (V3[2] + V3[5]) + (P2[2] * (+cI * (V3[2] + V3[5])) -
                                            P2[3] * (V3[3] + cI * (V3[4]))))) +
       (F1[3] * (P2[0] * (-V3[2] + V3[5]) + (P2[1] * (V3[3] - cI * (V3[4])) +
                                             (P2[2] * (+cI * (V3[3]) + V3[4]) +
                                              P2[3] * (-V3[2] + V3[5])))) +
        M2 * (F1[4] * (V3[3] + cI * (V3[4])) - F1[5] * (V3[2] + V3[5]))));
  F2[4] =
      denom * (-cI) *
      (F1[4] * (P2[0] * (-V3[2] + V3[5]) + (P2[1] * (V3[3] + cI * (V3[4])) +
                                            (P2[2] * (-cI * (V3[3]) + V3[4]) +
                                             P2[3] * (-V3[2] + V3[5])))) +
       (F1[5] * (P2[0] * (V3[3] - cI * (V3[4])) +
                 (P2[1] * (-one) * (V3[2] + V3[5]) +
                  (P2[2] * (+cI * (V3[2] + V3[5])) +
                   P2[3] * (V3[3] - cI * (V3[4]))))) +
        M2 * (F1[2] * (-one) * (V3[2] + V3[5]) +
              F1[3] * (-V3[3] + cI * (V3[4])))));
  F2[5] =
      denom * cI *
      (F1[4] *
           (P2[0] * (-one) * (V3[3] + cI * (V3[4])) +
            (P2[1] * (V3[2] - V3[5]) + (P2[2] * (+cI * (V3[2]) - cI * (V3[5])) +
                                        P2[3] * (V3[3] + cI * (V3[4]))))) +
       (F1[5] * (P2[0] * (V3[2] + V3[5]) +
                 (P2[1] * (-V3[3] + cI * (V3[4])) +
                  (P2[2] * (-one) * (+cI * (V3[3]) + V3[4]) -
                   P2[3] * (V3[2] + V3[5])))) +
