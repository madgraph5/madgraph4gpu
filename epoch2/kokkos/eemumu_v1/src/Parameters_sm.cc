...
void Parameters_sm::setIndependentParameters(SLHAReader& slha)
{
  ...
  zero = 0; 
  ZERO = 0; 
  // Prepare a std::vector for indices
...
void Parameters_sm::printIndependentCouplings()
{
  ...
  std::cout << std::setw(20) <<  "GC_50 " <<  "= " << std::setiosflags(std::ios::scientific) <<
      std::setw(10) << GC_50 << std::endl;
  std::cout << std::setw(20) <<  "GC_59 " <<  "= " << std::setiosflags(std::ios::scientific) <<
      std::setw(10) << GC_59 << std::endl;
}
