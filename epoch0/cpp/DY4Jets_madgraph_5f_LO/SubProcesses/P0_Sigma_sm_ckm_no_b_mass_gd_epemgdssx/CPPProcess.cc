...
// Process: g d > e+ e- g d s s~ WEIGHTED<=8 / h
// Process: g d > e+ e- g d b b~ WEIGHTED<=8 / h
// Process: g d > mu+ mu- g d s s~ WEIGHTED<=8 / h
// Process: g d > mu+ mu- g d b b~ WEIGHTED<=8 / h
// Process: g s > e+ e- g s d d~ WEIGHTED<=8 / h
// Process: g s > e+ e- g s b b~ WEIGHTED<=8 / h
// Process: g s > mu+ mu- g s d d~ WEIGHTED<=8 / h
// Process: g s > mu+ mu- g s b b~ WEIGHTED<=8 / h
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[5], w[9], pars->GC_1, pars->ZERO, pars->ZERO, w[15]); 
  FFV1P0_3(w[7], w[14], pars->GC_11, pars->ZERO, pars->ZERO, w[16]); 
  FFV1_2(w[1], w[9], pars->GC_1, pars->ZERO, pars->ZERO, w[17]); 
...
double CPPProcess::matrix_gd_epemgdssx_no_h() 
{
  int i, j; 
  // Local variables
