...
// Process: d d~ > e+ e- u d u~ d~ WEIGHTED<=8 / h
// Process: d d~ > e+ e- c d c~ d~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- u d u~ d~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- c d c~ d~ WEIGHTED<=8 / h
// Process: s s~ > e+ e- u s u~ s~ WEIGHTED<=8 / h
// Process: s s~ > e+ e- c s c~ s~ WEIGHTED<=8 / h
// Process: s s~ > mu+ mu- u s u~ s~ WEIGHTED<=8 / h
// Process: s s~ > mu+ mu- c s c~ s~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ddx_epemuduxdx_no_h(); 
        ...
        t[1] = matrix_ddx_epemuduxdx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[6], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[49]); 
...
double CPPProcess::matrix_ddx_epemuduxdx_no_h() 
{
  int i, j; 
  // Local variables
