...
// Process: c c~ > e+ e- u c u~ c~ WEIGHTED<=8 / h
// Process: c c~ > mu+ mu- u c u~ c~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ccx_epemucuxcx_no_h(); 
        ...
        t[1] = matrix_ccx_epemucuxcx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[0], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[31]); 
...
double CPPProcess::matrix_ccx_epemucuxcx_no_h() 
{
  int i, j; 
  // Local variables
