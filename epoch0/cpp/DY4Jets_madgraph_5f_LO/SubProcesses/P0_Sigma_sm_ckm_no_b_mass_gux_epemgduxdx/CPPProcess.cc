...
// Process: g u~ > e+ e- g d u~ d~ WEIGHTED<=8 / h
// Process: g u~ > e+ e- g s u~ s~ WEIGHTED<=8 / h
// Process: g u~ > e+ e- g b u~ b~ WEIGHTED<=8 / h
// Process: g u~ > mu+ mu- g d u~ d~ WEIGHTED<=8 / h
// Process: g u~ > mu+ mu- g s u~ s~ WEIGHTED<=8 / h
// Process: g u~ > mu+ mu- g b u~ b~ WEIGHTED<=8 / h
// Process: g c~ > e+ e- g d c~ d~ WEIGHTED<=8 / h
// Process: g c~ > e+ e- g s c~ s~ WEIGHTED<=8 / h
// Process: g c~ > e+ e- g b c~ b~ WEIGHTED<=8 / h
// Process: g c~ > mu+ mu- g d c~ d~ WEIGHTED<=8 / h
// Process: g c~ > mu+ mu- g s c~ s~ WEIGHTED<=8 / h
// Process: g c~ > mu+ mu- g b c~ b~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_gux_epemgduxdx_no_h(); 
        ...
        t[1] = matrix_gux_epemgduxdx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV2_4_3(w[2], w[3], -pars->GC_51, pars->GC_59, pars->mdl_MZ, pars->mdl_WZ,
      w[22]);
  FFV2_3_1(w[5], w[22], -pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[23]);
...
double CPPProcess::matrix_gux_epemgduxdx_no_h() 
{
  int i, j; 
  // Local variables
