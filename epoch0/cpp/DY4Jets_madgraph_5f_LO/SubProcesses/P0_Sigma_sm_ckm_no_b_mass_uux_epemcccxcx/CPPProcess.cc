...
// Process: u u~ > e+ e- c c c~ c~ WEIGHTED<=8 / h
// Process: u u~ > mu+ mu- c c c~ c~ WEIGHTED<=8 / h
// Process: c c~ > e+ e- u u u~ u~ WEIGHTED<=8 / h
// Process: c c~ > mu+ mu- u u u~ u~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uux_epemcccxcx_no_h(); 
        ...
        t[1] = matrix_uux_epemcccxcx_no_h(); 
        double tsum = 0; 
...
double CPPProcess::matrix_uux_epemcccxcx_no_h() 
{
  int i, j; 
  // Local variables
