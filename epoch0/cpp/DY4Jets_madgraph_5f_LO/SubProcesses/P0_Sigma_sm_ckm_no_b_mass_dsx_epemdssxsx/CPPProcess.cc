...
// Process: s d~ > e+ e- s d d~ d~ WEIGHTED<=8 / h
// Process: s d~ > mu+ mu- s d d~ d~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dsx_epemdssxsx_no_h(); 
        ...
        t[1] = matrix_dsx_epemdssxsx_no_h(); 
        double tsum = 0; 
...
double CPPProcess::matrix_dsx_epemdssxsx_no_h() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./4. * (+std::complex<double> (0, 1) * amp[16] +
