...
// Process: u~ c~ > e+ e- c u~ c~ c~ WEIGHTED<=8 / h
// Process: u~ c~ > mu+ mu- c u~ c~ c~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxcx_epemcuxcxcx_no_h(); 
        ...
        t[1] = matrix_uxcx_epemcuxcxcx_no_h(); 
        double tsum = 0; 
...
double CPPProcess::matrix_uxcx_epemcuxcxcx_no_h() 
{
  int i, j; 
  // Local variables
