...
// Process: u d > e+ e- u u d u~ WEIGHTED<=8 / h
// Process: u d > mu+ mu- u u d u~ WEIGHTED<=8 / h
// Process: u s > e+ e- u u s u~ WEIGHTED<=8 / h
// Process: u s > mu+ mu- u u s u~ WEIGHTED<=8 / h
// Process: u b > e+ e- u u b u~ WEIGHTED<=8 / h
// Process: u b > mu+ mu- u u b u~ WEIGHTED<=8 / h
// Process: c d > e+ e- c c d c~ WEIGHTED<=8 / h
// Process: c d > mu+ mu- c c d c~ WEIGHTED<=8 / h
// Process: c s > e+ e- c c s c~ WEIGHTED<=8 / h
// Process: c s > mu+ mu- c c s c~ WEIGHTED<=8 / h
// Process: c b > e+ e- c c b c~ WEIGHTED<=8 / h
// Process: c b > mu+ mu- c c b c~ WEIGHTED<=8 / h
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[7], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[25]); 
  FFV1P0_3(w[1], w[14], pars->GC_11, pars->ZERO, pars->ZERO, w[26]); 
  FFV1_1(w[5], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[27]); 
  ...
  FFV1_1(w[4], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[47]); 
  ...
  FFV1_2(w[0], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[54]); 
...
double CPPProcess::matrix_ud_epemuudux_no_h() 
{
  int i, j; 
  // Local variables
