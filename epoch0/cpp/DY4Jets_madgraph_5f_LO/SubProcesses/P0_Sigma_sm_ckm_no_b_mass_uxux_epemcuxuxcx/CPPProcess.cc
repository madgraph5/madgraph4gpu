...
// Process: u~ u~ > e+ e- c u~ u~ c~ WEIGHTED<=8 / h
// Process: u~ u~ > mu+ mu- c u~ u~ c~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxux_epemcuxuxcx_no_h(); 
...
double CPPProcess::matrix_uxux_epemcuxuxcx_no_h() 
{
  int i, j; 
  // Local variables
