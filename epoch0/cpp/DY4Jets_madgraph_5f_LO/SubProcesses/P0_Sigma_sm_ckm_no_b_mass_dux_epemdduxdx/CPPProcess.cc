...
// Process: d u~ > e+ e- d d u~ d~ WEIGHTED<=8 / h
// Process: d u~ > mu+ mu- d d u~ d~ WEIGHTED<=8 / h
// Process: d c~ > e+ e- d d c~ d~ WEIGHTED<=8 / h
// Process: d c~ > mu+ mu- d d c~ d~ WEIGHTED<=8 / h
// Process: s u~ > e+ e- s s u~ s~ WEIGHTED<=8 / h
// Process: s u~ > mu+ mu- s s u~ s~ WEIGHTED<=8 / h
// Process: s c~ > e+ e- s s c~ s~ WEIGHTED<=8 / h
// Process: s c~ > mu+ mu- s s c~ s~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dux_epemdduxdx_no_h(); 
        ...
        t[1] = matrix_dux_epemdduxdx_no_h(); 
        double tsum = 0; 
...
double CPPProcess::matrix_dux_epemdduxdx_no_h() 
{
  int i, j; 
  // Local variables
