...
// Process: d~ d~ > ta+ ta- u u~ d~ d~ WEIGHTED<=8 / h
// Process: d~ d~ > ta+ ta- c c~ d~ d~ WEIGHTED<=8 / h
// Process: s~ s~ > ta+ ta- u u~ s~ s~ WEIGHTED<=8 / h
// Process: s~ s~ > ta+ ta- c c~ s~ s~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dxdx_taptamuuxdxdx_no_h(); 
...
double CPPProcess::matrix_dxdx_taptamuuxdxdx_no_h() 
{
  int i, j; 
  // Local variables
