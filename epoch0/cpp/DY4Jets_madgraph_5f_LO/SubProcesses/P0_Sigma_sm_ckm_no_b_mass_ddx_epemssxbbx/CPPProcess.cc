...
// Process: d d~ > e+ e- s s~ b b~ WEIGHTED<=8 / h
// Process: d d~ > mu+ mu- s s~ b b~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ddx_epemssxbbx_no_h(); 
        ...
        t[1] = matrix_ddx_epemssxbbx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1P0_3(w[5], w[4], pars->GC_11, pars->ZERO, pars->ZERO, w[9]); 
  ...
  FFV1_0(w[37], w[1], w[48], pars->GC_11, amp[67]); 
  ...
  FFV1_0(w[43], w[1], w[48], pars->GC_11, amp[75]); 
...
double CPPProcess::matrix_ddx_epemssxbbx_no_h() 
{
  int i, j; 
  // Local variables
