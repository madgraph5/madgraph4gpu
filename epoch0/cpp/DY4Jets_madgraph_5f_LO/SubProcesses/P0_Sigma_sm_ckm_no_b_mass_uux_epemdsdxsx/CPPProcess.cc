...
// Process: u u~ > e+ e- d s d~ s~ WEIGHTED<=8 / h
// Process: u u~ > e+ e- d b d~ b~ WEIGHTED<=8 / h
// Process: u u~ > e+ e- s b s~ b~ WEIGHTED<=8 / h
// Process: u u~ > mu+ mu- d s d~ s~ WEIGHTED<=8 / h
// Process: u u~ > mu+ mu- d b d~ b~ WEIGHTED<=8 / h
// Process: u u~ > mu+ mu- s b s~ b~ WEIGHTED<=8 / h
// Process: c c~ > e+ e- d s d~ s~ WEIGHTED<=8 / h
// Process: c c~ > e+ e- d b d~ b~ WEIGHTED<=8 / h
// Process: c c~ > e+ e- s b s~ b~ WEIGHTED<=8 / h
// Process: c c~ > mu+ mu- d s d~ s~ WEIGHTED<=8 / h
// Process: c c~ > mu+ mu- d b d~ b~ WEIGHTED<=8 / h
// Process: c c~ > mu+ mu- s b s~ b~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uux_epemdsdxsx_no_h(); 
        ...
        t[1] = matrix_uux_epemdsdxsx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[1], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[41]); 
...
double CPPProcess::matrix_uux_epemdsdxsx_no_h() 
{
  int i, j; 
  // Local variables
