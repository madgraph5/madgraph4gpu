...
// Process: u d~ > e+ e- u s d~ s~ WEIGHTED<=8 / h
// Process: u d~ > e+ e- u b d~ b~ WEIGHTED<=8 / h
// Process: u d~ > mu+ mu- u s d~ s~ WEIGHTED<=8 / h
// Process: u d~ > mu+ mu- u b d~ b~ WEIGHTED<=8 / h
// Process: u s~ > e+ e- u d s~ d~ WEIGHTED<=8 / h
// Process: u s~ > e+ e- u b s~ b~ WEIGHTED<=8 / h
// Process: u s~ > mu+ mu- u d s~ d~ WEIGHTED<=8 / h
// Process: u s~ > mu+ mu- u b s~ b~ WEIGHTED<=8 / h
// Process: u b~ > e+ e- u d b~ d~ WEIGHTED<=8 / h
// Process: u b~ > e+ e- u s b~ s~ WEIGHTED<=8 / h
// Process: u b~ > mu+ mu- u d b~ d~ WEIGHTED<=8 / h
// Process: u b~ > mu+ mu- u s b~ s~ WEIGHTED<=8 / h
// Process: c d~ > e+ e- c s d~ s~ WEIGHTED<=8 / h
// Process: c d~ > e+ e- c b d~ b~ WEIGHTED<=8 / h
// Process: c d~ > mu+ mu- c s d~ s~ WEIGHTED<=8 / h
// Process: c d~ > mu+ mu- c b d~ b~ WEIGHTED<=8 / h
// Process: c s~ > e+ e- c d s~ d~ WEIGHTED<=8 / h
// Process: c s~ > e+ e- c b s~ b~ WEIGHTED<=8 / h
// Process: c s~ > mu+ mu- c d s~ d~ WEIGHTED<=8 / h
// Process: c s~ > mu+ mu- c b s~ b~ WEIGHTED<=8 / h
// Process: c b~ > e+ e- c d b~ d~ WEIGHTED<=8 / h
// Process: c b~ > e+ e- c s b~ s~ WEIGHTED<=8 / h
// Process: c b~ > mu+ mu- c d b~ d~ WEIGHTED<=8 / h
// Process: c b~ > mu+ mu- c s b~ s~ WEIGHTED<=8 / h
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_udx_epemusdxsx_no_h(); 
        ...
        t[1] = matrix_udx_epemusdxsx_no_h(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[0], w[10], pars->GC_2, pars->ZERO, pars->ZERO, w[37]); 
  ...
  FFV1_0(w[13], w[5], w[36], pars->GC_11, amp[49]); 
  FFV1_0(w[7], w[17], w[38], pars->GC_11, amp[50]); 
  FFV1_0(w[15], w[5], w[38], pars->GC_11, amp[51]); 
  FFV1_0(w[7], w[12], w[40], pars->GC_11, amp[52]); 
  FFV1_0(w[13], w[5], w[40], pars->GC_11, amp[53]); 
  FFV1_0(w[7], w[17], w[42], pars->GC_11, amp[54]); 
  FFV1_0(w[15], w[5], w[42], pars->GC_11, amp[55]); 
  FFV1_0(w[7], w[19], w[36], pars->GC_11, amp[56]); 
  FFV1_0(w[20], w[5], w[36], pars->GC_11, amp[57]); 
  FFV1_0(w[7], w[17], w[44], pars->GC_11, amp[58]); 
  FFV1_0(w[15], w[5], w[44], pars->GC_11, amp[59]); 
  FFV1_0(w[7], w[19], w[40], pars->GC_11, amp[60]); 
  FFV1_0(w[20], w[5], w[40], pars->GC_11, amp[61]); 
  FFV1_0(w[7], w[17], w[46], pars->GC_11, amp[62]); 
  FFV1_0(w[15], w[5], w[46], pars->GC_11, amp[63]); 
...
double CPPProcess::matrix_udx_epemusdxsx_no_h() 
{
  int i, j; 
  // Local variables
