...
// Process: g g > e+ e- u u u~ u~ WEIGHTED<=8 / h
// Process: g g > e+ e- c c c~ c~ WEIGHTED<=8 / h
// Process: g g > mu+ mu- u u u~ u~ WEIGHTED<=8 / h
// Process: g g > mu+ mu- c c c~ c~ WEIGHTED<=8 / h
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[4], w[9], pars->GC_2, pars->ZERO, pars->ZERO, w[17]); 
  ...
  FFV2_5_2(w[7], w[26], pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[29]);
  FFV2_5_1(w[4], w[26], pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[30]);
...
double CPPProcess::matrix_gg_epemuuuxux_no_h() 
{
  int i, j; 
  // Local variables
