...
// Process: u u~ > e+ e- g g c c~ WEIGHTED<=8 / h
// Process: u u~ > mu+ mu- g g c c~ WEIGHTED<=8 / h
// Process: c c~ > e+ e- g g u u~ WEIGHTED<=8 / h
// Process: c c~ > mu+ mu- g g u u~ WEIGHTED<=8 / h
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[0], w[9], pars->GC_2, pars->ZERO, pars->ZERO, w[17]); 
  ...
  FFV2_5_1(w[1], w[22], pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[25]);
  FFV2_5_2(w[0], w[22], pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[26]);
  FFV1P0_3(w[0], w[1], pars->GC_11, pars->ZERO, pars->ZERO, w[27]); 
...
double CPPProcess::matrix_uux_epemggccx_no_h() 
{
  int i, j; 
  // Local variables
  ...
  jamp[7] = +1./2. * (+std::complex<double> (0, 1) * amp[0] +
  ...
  jamp[8] = +1./2. * (-amp[96] - amp[97] - amp[98] - amp[99] - amp[100] -
  ...
  jamp[9] = +1./2. * (+1./3. * amp[96] + 1./3. * amp[97] + 1./3. * amp[98] +
